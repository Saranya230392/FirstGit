...
public class KeyDrawable extends Drawable {
    ...
    private void drawKeyModekey(Resources r, Canvas canvas) {
        ...
        if (keyboard.getKeyboardType() == DefaultSoftKeyboard.KEYBOARD_12KEY) {
            ...
            if (mVerticalDisp) {
                if (!mOneHandedShown) {
                    ...
                    textBlockSize02 = r.getDimensionPixelSize(R.dimen.key_mode_qwert_moji_text_size_vertical);
                    textBlockSize03 = r.getDimensionPixelSize(R.dimen.key_mode_qwert_text_size_vertical);
                } else {
                    textBlockSize01 = r.getDimensionPixelSize(R.dimen.key_onehanded_mode_qwert_moji_text_size_vertical);
                    textBlockSize02 = r.getDimensionPixelSize(R.dimen.key_onehanded_mode_qwert_moji_text_size_vertical);
                    textBlockSize03 = r.getDimensionPixelSize(R.dimen.key_onehanded_mode_qwert_text_size_vertical);
                }
            } else {
                textBlockSize01 = r.getDimensionPixelSize(R.dimen.key_mode_moji_text_size);
                textBlockSize02 = r.getDimensionPixelSize(R.dimen.key_mode_jp_on_text_size);
        ...
        setTextShadow(paintTextBlockMode, R.color.key_qwerty_text_color);
    ...
    private void drawKeyModekeyKo(Resources r, Canvas canvas) {
        ...
        positionX = 0;
        positionY = 0;
        mTextBlock01Size = OpenWnn.getContext().getResources().getDimensionPixelSize(R.dimen.key_mode_on_text_size);
        mPaintTextBlock01.setTextSize(mTextBlock01Size);
        mPaintTextBlock01.setColor(mKeySkin.getColor(OpenWnn.getContext(), R.color.key_text_color_2nd));

        setTextShadow(mPaintTextBlock01, R.color.key_qwerty_text_color);
    ...
    private void drawEisukanakey(Resources r, Canvas canvas) {
        ...
        mPaintTextBlock02.setTextAlign(Align.CENTER);
        setTextShadow(mPaintTextBlock01, R.color.key_qwerty_text_color);
        setTextShadow(mPaintTextBlock02, R.color.key_qwerty_text_color);
    ...
    private void drawPreviewkey(Resources r, Canvas canvas) {
        ...
        if (KEY_ID_ALPHA > offsetId || (KEY_ID_KATAKANA <= offsetId && offsetId < KEY_ID_NUM_HALF)) {
            mTextBlock01Size = r.getDimensionPixelSize(R.dimen.key_10key_text_size_small_top);
            mTextBlock02Size = r.getDimensionPixelSize(R.dimen.key_10key_text_size_left_right);
            mTextBlock03Size = r.getDimensionPixelSize(R.dimen.key_10key_text_size_left_right);
        ...
        if (mVerticalDisp && (KEY_ID_ALPHA <= offsetId)) {
            mPaintTextBlock02.setTextAlign(Align.CENTER);
        } else if (mId >= KEY_ID_PREVIEW_PHONE_NUM && mId < KEY_ID_PREVIEW_PHONE_SYM) {
            mPaintTextBlock02.setTextAlign(Align.CENTER);
        } else {
            mPaintTextBlock02.setTextAlign(Align.LEFT);
        }
        ...
        if (TEXTBLOCK_01[offsetId - 1] != null) {
            ...
            positionX = 0;
            if(mVerticalDisp){
                positionY = (float)(Math.abs(fontMetrics.descent + fontMetrics.ascent) - (float)mKeyHeight / 2  + m10KeyCenterTopVerticalGapY + offsetY);
            }else{
                positionY = (float)(Math.abs(fontMetrics.descent + fontMetrics.ascent) - (float)mKeyHeight / 2  + m10KeyCenterTopGapY + offsetY);
            }
        ...
        if (TEXTBLOCK_02[offsetId - 1] != null) {
            ...
            if(KEY_ID_ALPHA <= offsetId && offsetId < KEY_ID_KATAKANA ){
                if(!mVerticalDisp){
                    mPaintTextBlock02.setTextAlign(Align.CENTER);
                }
            }
            ...
            if (Align.CENTER == mPaintTextBlock02.getTextAlign()) {
                positionX = 0;
                positionY = (float)(mKeyHeight * mLeftCenterGap) - (float)mKeyHeight / 2 -
                            (float)(Math.abs(fontMetrics.descent + fontMetrics.ascent)) / 2 + offsetY;
                canvas.drawText(TEXTBLOCK_02[offsetId - 1], positionX, positionY, mPaintTextBlock02);
            } else {
                fontMetrics = mPaintTextBlock01.getFontMetrics();
                if(mVerticalDisp){
                    positionX = (float)(m10KeyLeftVerticalGapX - mKeyWidth / 2 );
                    positionY = (float)(Math.abs(fontMetrics.ascent + fontMetrics.descent) / 2
                                             + m10KeyCenterBottomVerticalGapY + offsetY);
                }else{
                    if (mOneTouchEmojiShown) {
                        positionX = (float)(m10KeyLeftGapX - mKeyWidth / 2);
                    } else {
                        positionX = (float)(m10KeyLeftEmojioffGapX - mKeyWidth / 2);
                    }
                    positionY = (float)(Math.abs(fontMetrics.ascent + fontMetrics.descent) / 2
                                             + m10KeyCenterBottomGapY + offsetY);
                }
                canvas.drawText(TEXTBLOCK_02[offsetId - 1], positionX, positionY, mPaintTextBlock02);
            }
        }
        ...
        if (TEXTBLOCK_03[offsetId - 1] != null) {
            ...
            if(mVerticalDisp){
                ...
                if (mOneTouchEmojiShown) {
                    ...
                    positionX = (float)(mKeyWidth / 2 - m10KeyRightEmojioffGapX );
                }
                positionY = (float)(Math.abs(fontMetrics.ascent + fontMetrics.descent) / 2
                                    + m10KeyCenterBottomGapY + offsetY);
            }
    ...
    private void drawPreview12KeyTenkey(Resources r, Canvas canvas) {
        ...
        if (TEXTBLOCK_01[offsetId - 1] != null) {
            fontMetrics = mPaintTextBlock01.getFontMetrics();
            positionX = 0;
            positionY = 0 + (float)(Math.abs(fontMetrics.descent + fontMetrics.ascent)/2) + offsetY;
            canvas.drawText(TEXTBLOCK_01[offsetId - 1], positionX, positionY, mPaintTextBlock01);
        }
    }
