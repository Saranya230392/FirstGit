...
public class KeyDrawable extends Drawable {
    ...
    private void drawNumkey(Resources r, Canvas canvas) {
        ...
        if (KEY_ID_NUM_HALF <= mId && mId < KEY_ID_SYM) {
            ...
            mPaintTextBlock01.setColor(mKeySkin.getColor(OpenWnn.getContext(), R.color.key_10key_text_phone_large_top_color));
            mPaintTextBlock02.setColor(mKeySkin.getColor(OpenWnn.getContext(), R.color.key_10key_text_phone_left_right_color));
        }
        //Gap
        int topGapY = 0;
        int bottomGapY = 0;
        if ((KEY_ID_NUM_HALF <= mId && mId < KEY_ID_SYM)
                || (KEY_ID_PHONE_NUM_ZERO < mId && mId < KEY_ID_PHONE_SYM)){
            if (mVerticalDisp) {
        ...
        } else if (KEY_ID_PHONE_NUM <= mId && mId <= KEY_ID_PHONE_NUM_ZERO) {
            if (mVerticalDisp) {
    ...
    private void drawKeyModekey(Resources r, Canvas canvas) {
        ...
        if (TEXTBLOCK_02[mId - 1] != null) {
            ...
            paintTextBlockMode.setTextAlign(Align.CENTER);
            float textBlockSize = (TEXTBLOCK_03[mId - 1] != null) ? textBlockSize02 : textBlockSize03;
            paintTextBlockMode.setTextSize(textBlockSize);
            paintTextBlockMode.setAntiAlias(true);

            float positionX_left = positionX;
    ...
    private void drawSymkey(Resources r, Canvas canvas) {
        ...
        boolean isQwerty = (keyboard.getKeyboardType() == DefaultSoftKeyboard.KEYBOARD_QWERTY);
        boolean isSmall = false;
        if (mVerticalDisp && isQwerty && (mOneTouchEmojiShown || mOneHandedShown)) {
            isSmall = true;
        }
        setOneTouchEmojiShown();
        if(!mKeySkin.isValid()){
