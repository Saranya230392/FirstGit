...
public class BaseHandwritingKeyboard extends HandwritingKeyboard {
    private static final String TAG = "LGHWIMEBaseHandwritingKeyboard";
    public static final boolean DEBUG = HandwritingKeyboard.DEBUG;
    public static final boolean DEBUG_PROCESS = HandwritingKeyboard.DEBUG;
...
//			0x3147, 0x3148, 0x314a, 0x314b, 0x314c, 0x314d, 0x314e };
    ...
    class WrapWnnEngine extends jp.co.omronsoft.iwnnime.ml.WrapWnnEngine {
    	...
    	private boolean mFromSetCandidates;
...
//			}
    ...
    public void onCreate() {
    	if (DEBUG_PROCESS) Log.w(TAG, "onCreate");
...
//        changeMode(InputMethodSwitcher.IME_TYPE_EMOJIPALLETE);
    ...
    private void createInputView() {
        createInputView(R.layout.handwriting_ime);
    }
    
    private void createInputView(int layoutResId) {
        getPrefSettings();
        mThemeManager = new ThemeManager(getApplicationContext());

        mInputView = (LinearLayout) mThemeManager.inflate(layoutResId, null);
    ...
    public View onCreateInputView() {
    	if (DEBUG_PROCESS) Log.w(TAG, "onCreateInputView");

//      getPrefFloating();
    	int resId = R.layout.handwriting_ime;
    	if (isEnableFloating()) {
    		resId = R.layout.handwriting_ime_floating;
    	}
    	createInputView(resId);
    	
        return mInputView;
    }
    
    private void splitMode() {
        Log.e(TAG, "isSplitViewMode() = " + isSplitViewMode()
        		+ " isEnableFloating() = " + isEnableFloating()
        		+ " mInputView=" + mInputView);
        
    	openFloatingKeyboardHW((ViewGroup) mInputView.getParent(), mInputView);
    }
	...
	public void setInputView(View view) {
		if (DEBUG_PROCESS) Log.w(TAG, "setInputView");
		super.setInputView(view);
	}
	
//	private void getPrefFloating() {
//		SharedPreferences pref = PreferenceManager.getDefaultSharedPreferences(getBaseContext());
//		Resources res = getResources();
//		
//		setEnableFloating(pref.getBoolean(getFloatingPreferenceKeyName(),
//                res.getBoolean(R.bool.floating_mode_show_default_value)));
//	}
	
	// TODO : couldn't get isEnableFloating() properly.
    @Override
	public boolean isEnableFloating() {
        ...
        boolean isEnableFloating = pref.getBoolean(getFloatingPreferenceKeyName(), OpenWnn.superGetContext()
                .getResources().getBoolean(R.bool.floating_mode_show_default_value));
        
		Log.e("JM", "super.isEnableFloating()=" + super.isEnableFloating() + " isEnableFloating()=" + isEnableFloating);
		return isEnableFloating;
	}

	@Override
    public void onStartInputView(EditorInfo info, boolean restarting) {
		if (DEBUG_PROCESS) Log.w(TAG, "onStartInputView");
		
//		getPrefFloating();
			
		if(isSplitViewMode() && isEnableFloating()) {
			if (mInputView.getTag().equals("inputView") || mThemeManager == null || mThemeManager.isThemeChanged()) {
				createInputView(R.layout.handwriting_ime_floating);
				setInputView(mInputView);
			}
		} else 
			if (mInputView.getTag().equals("floatingInputView") || mThemeManager == null || mThemeManager.isThemeChanged()) {
			createInputView();
			setInputView(mInputView);
		}
...
//        mButtonLayoutManager.setPrefSettingIcon();
    ...
    public void onFinishInputView(boolean finishingInput) {
        if (DEBUG_PROCESS) Log.d(TAG, "[onFinishInputView()] mModeName=" + mModeName +
        ...
        if (mIsCandidatesViewShown) {
        	// NOTE : seCandidatesViewVisibility() has more functions so cannot be used here.
        	mCandidatesViewManager.clearCandidates();
        	mIsCandidatesViewShown = false;
        }
        dismissAllPopups();
    ...
    public void onWindowShown() {
        super.onWindowShown();
        if (DEBUG_PROCESS) Log.w(TAG, "onWindowShown");
...
//        mModeManager.setAutoScrollWidth(mNextScreenWidth);
    ...
    public View onCreateCandidatesView() {
    	if (DEBUG_PROCESS) Log.w(TAG, "onCreateCandidatesView");
...
//
        ...
        if (isEnableFloating()) {
        	int candidateWidth = getResources (). getDimensionPixelSize(R.dimen.floating_keyboard_width);
        	int candidateHeight = getResources (). getDimensionPixelSize(R.dimen.floating_keyboard_height);
        	mCandidatesView = mCandidatesViewManager.initView(this, candidateWidth, candidateHeight);
        	mCandidatesViewManager.setViewType(WrapTextCandidatesViewManager.VIEW_TYPE_NORMAL);
        	if (DEBUG) Log.d(TAG, "onCreateCandidtaesView size=" + candidateWidth + ", " + candidateWidth);
        } else {
    ...
    // TODO : This method has some button-related process. it makes candidates view hiding slow.
    //        In this reason, onFinishInput should use internal method "clearCandidates()" directly.
    @Override
    public void setCandidatesViewVisibility(boolean shown) {
    	if (DEBUG) Log.d(TAG, "setCandidatesViewVisibility shown=" + shown + " isInputViewShown()=" + isInputViewShown());
        ...
        if (shown) {
            if (isInputViewShown()) {
                ...
                mButtonLayoutManager.changeEnterState(ButtonLayoutManager.STATE_ENTER_OK);
            }
            mCandidatesViewManager.setEnableCandidateLongClick(
                    mModeName.equals(getString(R.string.HW_MODE_TEXT)));
        } else {
            mCandidatesViewManager.clearCandidates();
            mButtonLayoutManager.changeEnterState(ButtonLayoutManager.STATE_ENTER_ENTER);
        }
        
        // arrange buttons
        setButtonEnabled("space", !shown);
    }
    
    @Override
    public boolean isCandidatesViewVisible() {
    	if (mCandidatesViewManager == null || mCandidatesView == null) {
    		return false;
    	}
...
//        mCandidatesViewManager.setWnnEngine(mDictionaryEngine);
    ...
    public void onComputeInsets(Insets outInsets) {
        ...
        if (DEBUG_PROCESS) Log.w(TAG, "onComputeInsets");
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
    	if (DEBUG_PROCESS) Log.w(TAG, "onKeyDown " + keyCode);
...
//        boolean isSoundOn = (soundDefaultId != 0) ? getResources().getBoolean(vibDefaultId) : true;
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        if (DEBUG_PROCESS) Log.i(TAG, "onConfigurationChanged() " + newConfig.orientation);
        dismissAllPopups();
    ...
    public void onUpdateSelection(int oldSelStart, int oldSelEnd,
        ...
        if (DEBUG_PROCESS) Log.d(TAG, "oldSelStart=" + oldSelStart + " oldSelEnd=" + oldSelEnd +
    ...
    public synchronized boolean onEvent(HWEvent ev) {
    	if (DEBUG_PROCESS) Log.w(TAG, "onEvent " + ev); 
    	
        switch(ev.code){
        case HWEvent.CHANGE_FLOATING_KEYBOARD:
        	Log.e(TAG, "Change floating Keyboard!");
        ...
        default :
            if (DEBUG) Log.i(TAG, "HWEvent " + ev.code + " is unkown event!");
            break;
        }
    ...
    public void onDestroy() {
        if (DEBUG_PROCESS) Log.i(TAG, "onDestroy()");
        if (mModeManager != null) {
            mModeManager.clear();
        }
        mCore.destroy();
