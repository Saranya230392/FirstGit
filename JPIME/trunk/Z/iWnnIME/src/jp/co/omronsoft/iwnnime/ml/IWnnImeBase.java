...
public class IWnnImeBase {
    ...
    /** Candidates view shown floating keyboard */
    private boolean mCandidatesViewShownFloating = false;
    ...
    protected void showVoiceWarningDialog(final boolean swipe) {
        AlertDialog.Builder builder = new AlertDialog.Builder(OpenWnn.superGetContext(), R.style.Dialog);
        builder.setCancelable(true);
        builder.setIcon(R.drawable.ic_dialog_voice_input);
        ...
        String message = getResources().getString(R.string.ti_voice_warning_may_not_understand_txt) + "\n\n" +
        ...
        if (InputMethodBase.mOriginalInputMethodSwitcher.mInputMethodBase
            ...
            lp.token = InputMethodBase.mOriginalInputMethodSwitcher.mInputMethodBase
                    .getFloatingPopupParent().getWindowToken();
        } else {
     ...
     *
     * @param view
     * @return true: this method proceeded. false: this method do nothing
     */
    public boolean setCandidatesView(View view) {
        if (InputMethodBase.mOriginalInputMethodSwitcher.isEnableFloating()) {
            View floatingKeyboard = mWnnSwitcher.getFloatingContentView();
            if (floatingKeyboard != null) {
                LinearLayout candidateArea = (LinearLayout)floatingKeyboard.
                        findViewById(R.id.candidate_area);
                candidateArea.removeAllViews();
                candidateArea.addView(view);
                return true;
            }
        }
        return false;
    ...
    /**
     * Set whether candidate view shown or not.
     *
     * @param shown show or not
     * @return true: this method proceeded. false: this method do nothing
     */
   public boolean setCandidatesViewShown(boolean shown) {
        if (InputMethodBase.mOriginalInputMethodSwitcher.isEnableFloating()) {
            if (shown) {
                getCandidatesViewManager().getCurrentView().setVisibility(View.VISIBLE);
                mCandidatesViewShownFloating = true;
            } else {
                getCandidatesViewManager().getCurrentView().setVisibility(View.GONE);
                mCandidatesViewShownFloating = false;
            }
            return true;
        }
        return false;
    ...
    public void showFloating() {
        if (InputMethodBase.mOriginalInputMethodSwitcher.isEnableFloating() && (
                mWnnSwitcher.getApplicationContext().getResources().getConfiguration()
                .orientation == Configuration.ORIENTATION_LANDSCAPE)) {
            onCreateInputViewOpenWnn();
        }
        DefaultSoftKeyboard inputViewManager = (DefaultSoftKeyboard)getInputViewManager();
        mWnnSwitcher.openFloatingKeyboard(new LinearLayout(mWnnSwitcher.getApplicationContext()),
                inputViewManager.getCurrentView(), getCandidatesViewManager().getCurrentView());

        if (InputMethodBase.mOriginalInputMethodSwitcher.isEnableFloating()) {
            inputViewManager.getKeyboardView().setPopupParent(mWnnSwitcher.getFloatingPopupParent());
        } else {
            inputViewManager.getKeyboardView().setPopupParent(inputViewManager.getKeyboardView());
        }
    }

    /**
     * Get candidate view shown(only floating keyboard).
     *
     * @return true: candidate view. false: not candidate view
     */
    public boolean isCandidatesViewShownFloating() {
        return mCandidatesViewShownFloating;
    }
