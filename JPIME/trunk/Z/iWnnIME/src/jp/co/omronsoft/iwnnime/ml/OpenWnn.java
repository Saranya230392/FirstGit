...
public class OpenWnn extends InputMethodBase {
    ...
    */
    ...
    public void setCandidatesView(View view) {
        if (!mWnn.setCandidatesView(view)) {
    ...
    @Override public void setCandidatesViewShown(boolean shown) {
        ...
        if (!mWnn.setCandidatesViewShown(shown)) {
            super.setCandidatesViewShown(shown);
        }
        if (shown) {
            showWindow(true);
            ...
            extractEditText.addTextChangedListener(new TextWatcher() {
                ...
                public void onTextChanged(CharSequence s, int start, int before, int count) {
                    ...
                    if (ret > EmojiAssist.TYPE_TEXT_EMOJI) {
                        ...
                        String emojiMode = pref.getString("opt_one_touch_emoji_list", "hidden");
                        if (emojiMode.equals("hidden")) {
                            endEmojiAssist();
                        }
                    }
                }
    ...
    public boolean isSubtypeExtraEmojiInput() {
        ...
        if (manager != null) {
            ...
            if ((subtypeExtra != null) && subtypeExtra.equals(SUBTYPE_EMOJI_INPUT_EXTRA_VALUE)) {
                return true;
            }
        }
        return false;
        ...
        */
        public void run() {
            ...
            if (InputMethodBase.mOriginalInputMethodSwitcher.mInputMethodBase
                    .isNotExpandedFloatingMode()) {
                view = InputMethodBase.mOriginalInputMethodSwitcher.mInputMethodBase
                        .getFloatingPopupParent();
                SharedPreferences sharedPref = PreferenceManager
                        .getDefaultSharedPreferences(getContext());
                if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT) {
                    mParentLocation[0] = sharedPref.getInt(
                            InputMethodBase.FLOATING_POPUP_X_PORT_KEY, getResources().
                            getDimensionPixelSize(R.dimen.floating_keyboard_default_pos_x));
                    mParentLocation[1] = sharedPref.getInt(
                ...
                mParentLocation[1] += getResources().getDimensionPixelSize(
                        R.dimen.floating_keyboard_actionbar_height);
            } else {
                if (getResources().getConfiguration().orientation
                        == Configuration.ORIENTATION_LANDSCAPE) {
                    int candidateMinimumHeight = getResources()
                            .getDimensionPixelSize(R.dimen.cand_minimum_height);
                    int mCandidateBgViewPaddingTop = getResources()
                            .getDimensionPixelSize(R.dimen.cand_base_view_padding_top);
                    locationAdjustY = locationAdjustY - candidateMinimumHeight
                            - mCandidateBgViewPaddingTop + getResources().getDimensionPixelSize(
                            R.dimen.composing_extra_view_offset_y);
                    if (mParentLocation[1] > 0) {
                        locationAdjustY = 0;
                    }
                }
                view = (View) mCandidatesViewManager.getCurrentView();
                view.getLocationInWindow(mParentLocation);
                mParentLocation[1] += locationAdjustY;
            }
