...
public class FlickKeyboardView extends KeyboardView {
    ...
    @Override public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (mTouchKey != null 
                        && mTouchKey.isSplitKey() && !mTouchKey.isInsideSplitKey(mPressX, mPressY)) {
                    return false;
                }
    ...
    private void showFlickPopup(CharSequence label, Drawable image, boolean flickable) {
        ...
        int popupHeight = previewText.getBackground().getIntrinsicHeight();
        ...
        if (label == null) {
            ...
            if (i == null && !flickable) {
                ...
                if (i != null) {
                    ...
                    previewText.setCompoundDrawables(null, null, null, i);
                    int bottom = res.getDimensionPixelSize(R.dimen.preview_flick_padding_bottom);
                    if (!mShowFlickPopup && (key.codes[0] == DefaultSoftKeyboard.KEYCODE_JP12_EMOJI)) {
                        // Adjusting alignment of key preview of emoji key on 12 key keyboard except emoji flick preview.
                        bottom = res.getDimensionPixelSize(R.dimen.preview_flick_padding_bottom_symbol);
                    }
                    mPreviewText.setPadding(
        ...
        if(mShowFlickPopup){
            if (key.codes[0] == DefaultSoftKeyboard.KEYCODE_JP12_EMOJI) {
                popupWidth = res.getDimensionPixelSize(R.dimen.key_emoji_change_width);
                popupHeight = res.getDimensionPixelSize(R.dimen.key_emoji_change_height);
            } else {
        ...
        if ((key.edgeFlags & Keyboard.EDGE_LEFT) > 0) {
            if (mShowFlickPopup) {
                ...
                popupPreviewX = (int) getX() + key.x + getPaddingLeft();
            }
        } else if ((key.edgeFlags & Keyboard.EDGE_RIGHT) > 0) {
            popupPreviewX = (int) getX() + key.x - popupWidth + key.width + getPaddingLeft();
        } else {
        ...
        if (previewPopup.isShowing()) {
            ...
            View parentView;
            if (InputMethodBase.mOriginalInputMethodSwitcher.
                    mInputMethodBase.isNotExpandedFloatingMode()) {
                parentView = InputMethodBase.mOriginalInputMethodSwitcher.
                        mInputMethodBase.getFloatingPopupParent();
            } else {
