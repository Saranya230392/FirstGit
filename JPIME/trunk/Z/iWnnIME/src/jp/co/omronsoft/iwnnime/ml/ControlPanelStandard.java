...
public class ControlPanelStandard extends InputMethodSettingsActivity
    ...
    /** Key of "input language ja" setting. */
    public static final String INPUT_LANGUAGE_JA_KEY = "input_language_ja";

    /** Key of "input language en" setting. */
    public static final String INPUT_LANGUAGE_EN_KEY = "input_language_en";

    /** Key of "input language ko" setting. */
    public static final String INPUT_LANGUAGE_KO_KEY = "input_language_ko";
    ...
    private static class PreferenceShutterManager {
        ...
        public void changeShowingByCategoryDictionary(Context context) {
            ...
            // These are always hidden in LGE customization.
            if (!enableAdditional && !enableDownload) {
                showPreferenceByKey(CATEGORY_DICTIONARY_KEY, false);
            } else {
                showPreferenceByKey(CATEGORY_DICTIONARY_KEY, false);
            }
            showPreferenceByKey(ADDITIONAL_DICTIONARY_KEY, false);
            showPreferenceByKey(DOWNLOAD_DICTIONARY_KEY, false);
        }
    }
    ...
    @Override public void onCreate(Bundle savedInstanceState) {
        ...
        mChangeListener = new OnSharedPreferenceChangeListener() {
                public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
                    ...
                    } else if (key.equals(INPUT_LANGUAGE_JA_KEY)
                            || key.equals(INPUT_LANGUAGE_EN_KEY)
                            || key.equals(INPUT_LANGUAGE_KO_KEY)) {
                        changeShowingByInputLanguage();
                    } else if (key.equals(CANDIDATE_LINES_PORTRAIT_KEY)) {
                        setSummaryOfCandidateLinsPortrait();
                    }// else {}
                }
            };
    ...
    private void setDisableFunctionOneHanded() {
        ...
        editor.putBoolean("one_handed_show" + splitViewModeString, false);
        editor.commit();
    }
    ...
    private void setDisableFunctionSplit() {
        ...
        SharedPreferences sharedPref = PreferenceManager.getDefaultSharedPreferences(this);
        SharedPreferences.Editor editor = sharedPref.edit();
        editor.putBoolean("split_mode_show" + splitViewModeString, false);
        editor.commit();
    }
    ...
    /**
     * Change showing of preference(input language).
     */
    private void changeShowingByInputLanguage() {
        SharedPreferences preference = PreferenceManager.getDefaultSharedPreferences(this);
        boolean enableJa = true;
        boolean enableEn = true;
        boolean enableKo = true;

        enableJa = preference.getBoolean(INPUT_LANGUAGE_JA_KEY,
                mResources.getBoolean(R.bool.input_language_ja_default_value));
        enableEn = preference.getBoolean(INPUT_LANGUAGE_EN_KEY,
                mResources.getBoolean(R.bool.input_language_en_default_value));
        enableKo = preference.getBoolean(INPUT_LANGUAGE_KO_KEY,
                mResources.getBoolean(R.bool.input_language_ko_default_value));

        int enableCnt = 0;
        enableCnt += enableJa ? 1 : 0;
        enableCnt += enableEn ? 1 : 0;
        enableCnt += enableKo ? 1 : 0;

        Preference preferenceScreen = null;
        if (enableCnt > 1) {
            preferenceScreen = getPreferenceScreen().findPreference(INPUT_LANGUAGE_JA_KEY);
            if (preferenceScreen != null) {
                preferenceScreen.setEnabled(true);
            }
            preferenceScreen = getPreferenceScreen().findPreference(INPUT_LANGUAGE_EN_KEY);
            if (preferenceScreen != null) {
                preferenceScreen.setEnabled(true);
            }
            preferenceScreen = getPreferenceScreen().findPreference(INPUT_LANGUAGE_KO_KEY);
            if (preferenceScreen != null) {
                preferenceScreen.setEnabled(true);
            }
            return;
    ...
    /**
     * Set the text for the summary of the candidate lines of portrait.
     *
     */
    private void setSummaryOfCandidateLinsPortrait() {
        Resources res = getResources();
        SharedPreferences sharedPref = PreferenceManager.getDefaultSharedPreferences(this);
        String portraitLines = sharedPref.getString(ControlPanelStandard.CANDIDATE_LINES_PORTRAIT_KEY,
                res.getString(R.string.setting_portrait_default_value));
        setSummary(CANDIDATE_LINES_PORTRAIT_KEY, portraitLines.toString());
    }
