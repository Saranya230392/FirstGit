...
///////////////// iwnn_trial_add_2
    ...
    /**
     * Implementation of IVoiceEngineService.
     */
    private IVoiceEngineService.Stub mEngineService = new IVoiceEngineService.Stub() {
            /**
...
///////////////// iwnn_trial_add_3
           ...
           /**
            ...
            public boolean setDictionary(String packageName, String configurationFile, int language, int dictionary, boolean flexibleSearch,
                ...
                synchronized (this) {
                    if (DEBUG) Log.d(TAG, "IWnnEngineService::setDictionary(" + configurationFile + "," + language + "," + dictionary
                            + "," + flexibleSearch + "," + tenKeyType + "," + emojiFilter + "," + emailFilter + "," + convertCandidates
                            + "," + learnNumber + ")[" + getEngineService(packageName).mEngineStatus + "]");
            ...
            public boolean setDictionaryDecoratedPict(String packageName, String configurationFile, int language,
                ...
                synchronized (this) {
                    if (DEBUG) Log.d(TAG, "IWnnEngineService::setDictionaryDecoratedPict("
                    ...
                    if (ENGINE_STATUS_CHECK_TABLE[FUNCTION_SETDICTIONARY][engineService.mEngineStatus]) {
                        ...
                        if (isDictionaryChanged) {
                            ...
                            if (!engine.setDictionary(language, dictionary, this.hashCode(),
                                            configurationFile, null, engineService.mPassword)) {
                                engineService.mErrorCode = ERR_COMMON;
                                return false;
                            } else {
            ...
            public boolean setGijiFilter(String packageName, int[] type) {
                synchronized (this) {
                    if (DEBUG) Log.d(TAG, "IWnnEngineService::setGijiFilter(" + packageName + ")[" + getEngineService(packageName).mEngineStatus + "]");
                    EngineService engineService = getEngineService(packageName);
                    if (engineService == null) {
                        return false;
                    }
            ...
            public boolean undo(String packageName) {
                synchronized (this) {
                    if (DEBUG) Log.d(TAG, "IWnnEngineService::undo()[" + getEngineService(packageName).mEngineStatus + "]");
                    EngineService engineService = getEngineService(packageName);
                    if (engineService == null ) {
                        return false;
                    }
            ...
            public void startInput(String packageName) {
                synchronized (this) {
                    if (DEBUG) Log.d(TAG, "IWnnEngineService::startInput()[" + getEngineService(packageName).mEngineStatus + "]");
                    EngineService engineService = getEngineService(packageName);
                    if (engineService == null ) {
                        return;
                    }
            ...
            public boolean isGijiDic(String packageName, int index) {
                synchronized (this) {
                    if (DEBUG) Log.d(TAG, "IWnnEngineService::isGijiDic(" 
                                              + packageName + "," + index
                                              + ")[" + getEngineService(packageName).mEngineStatus + "]");
                    EngineService engineService = getEngineService(packageName);
                    if (engineService == null ) {
            ...
            public int getStatus(String packageName) {
                synchronized (this) {
                    if (DEBUG) Log.d(TAG, "IWnnEngineService::getStatus()[" + getEngineService(packageName).mEngineStatus + "]");
                    EngineService engineService = getEngineService(packageName);
                    if (engineService == null ) {
            ...
            public int getDictionaryType(String packageName) {
                synchronized (this) {
                    if (DEBUG) Log.d(TAG, "IWnnEngineService::getDictionaryType()[" + getEngineService(packageName).mEngineStatus + "]");
                    EngineService engineService = getEngineService(packageName);
                    if (engineService == null ) {
                        return -1;
                    }
            ...
            public int getErrorCode(String packageName) {
                synchronized (this) {
                    if (DEBUG) Log.d(TAG, "IWnnEngineService::getErrorCode()[" + getEngineService(packageName).mEngineStatus + "]");
                    EngineService engineService = getEngineService(packageName);
                    if (engineService == null ) {
                        return ERR_INVALID_STATE;
                    }
    ...
    private EngineService getEngineService(String packageName) {
        ...
        for (int i = 0; i < mEngineServiceArray.size(); i++) {
            if (mEngineServiceArray.get(i).mConnectedPackage.equals("")) {
