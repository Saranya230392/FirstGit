...
import jp.co.omronsoft.iwnnime.ml.DownloadDictionaryPreferenceActivity.DownloadDictionary;
...
public class ControlPanelStandard extends InputMethodSettingsActivity
    ...
    private void setSummaryOfDownloadDictionary() {
        Resources res = getResources();
        ArrayList<DownloadDictionary> cp = DownloadDictionaryPreferenceActivity.getDictionaryFromContentProvider(this);
        ArrayList<DownloadDictionary> fp = DownloadDictionaryPreferenceActivity.checkDictionaryFilePath(cp);
        ArrayList<DownloadDictionary> sp = DownloadDictionaryPreferenceActivity.readDownloadDictionaryFromSharedPreferences(this);
        ArrayList<DownloadDictionary> dictionary = DownloadDictionaryPreferenceActivity.checkConsistency(fp, sp, this);

        Iterator<DownloadDictionary> it = dictionary.iterator();
        int settingCnt = 0;
        while (it.hasNext()) {
            DownloadDictionary dic = it.next();
            for (int cnt = 0; cnt < DownloadDictionaryPreferenceActivity.MAX_DOWNLOAD_DIC; cnt++) {
                String file = OpenWnn.getStringFromNotResetSettingsPreference(this,
                                                                               DownloadDictionaryPreferenceActivity.createSharedPrefKey(cnt, DownloadDictionaryPreferenceActivity.QUERY_PARAM_FILE),
                                                                               null);
        ...
        int max = DownloadDictionaryPreferenceActivity.MAX_DOWNLOAD_DIC;
        String summary = res.getString(R.string.ti_preference_multiple_selection_summary_txt, settingCnt, max);
