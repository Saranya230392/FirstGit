...
public class OneTouchEmojiListViewManager {
    ...
    /** DecoEmoji row number list of 1stView for mixed display mode on split qwerty */
    private ArrayList<Integer> mMixedDecoEmojiIndexList1stSplitQwerty = new ArrayList<Integer>();
    /** Emoji row number list of 1stView for mixed display mode on split qwerty */
    private ArrayList<Integer> mMixedEmojiIndexList1stSplitQwerty = new ArrayList<Integer>();
    ...
    private TextView createCandidateView(int status) {
        ...
        if ((status == VIEW_TYPE_NORMAL) && (WnnKeyboardFactory.getSplitMode())) {
            ...
            // VIEW_TYPE_NORMAL
            text.setMinimumWidth(mWnn.getResources().getDimensionPixelSize(
                    R.dimen.one_touch_emoji_list_width_full));
        }
        return text;
    }
    ...
    private void setViewLayout(int type) {
        ...
        case VIEW_TYPE_FULL:
            ...
            if (WnnKeyboardFactory.getSplitMode()) {
                ...
                if (Configuration.ORIENTATION_LANDSCAPE == r.getConfiguration().orientation) {
                    int paddingLeft =
                            r.getDimensionPixelSize(R.dimen.one_touch_emoji_list_read_more_padding_left_full);
                    int paddingRight =
                            r.getDimensionPixelSize(R.dimen.one_touch_emoji_list_read_more_padding_right_full);
                    if (DefaultSoftKeyboardJAJP.ONETOUCHEMOJILIST_LEFT.equals(mListMode)) {
                        mReadMoreButton.setPadding(
                                paddingLeft,
                                mReadMoreButton.getPaddingTop(),
        ...
        while((word = mConverterSymbolEngine.getNextCandidate()) != null) {
            ...
            if (m1stViewCount <= max1stView) {
                ...
                case MODE_ONETOUCHEMOJI_MIXED_DECOEMOJI:
                    if (isSplitQwerty()) {
                        index1stView = mMixedDecoEmojiIndexList1stSplitQwerty
                                .get(m1stViewCount - mMixedHistoryIndex1st);
                    } else {
                        index1stView = mMixedDecoEmojiIndexList1st
                                .get(m1stViewCount - mMixedHistoryIndex1st);
                    }
                    if (index1stView == 0) {
                ...
                case MODE_ONETOUCHEMOJI_MIXED_EMOJI:
                    if (isSplitQwerty()) {
                        index1stView = mMixedEmojiIndexList1stSplitQwerty
                                .get(m1stViewCount - mMixedDecoEmojiIndex1st);
                    } else {
                        index1stView = mMixedEmojiIndexList1st
                                .get(m1stViewCount - mMixedDecoEmojiIndex1st);
                    }
                    index1stView = index1stView + mMixedHistoryIndex1st + (mMixedHistoryIndex1st == 0 ? 0 : (- 1));
                    break;
            ...
            if (m2ndViewCount <= max2ndView) {
                ...
                if (index2ndView <= mIndexMax2nd) {
                    ...
                    if (((index2ndView == 0) && !WnnKeyboardFactory.getSplitMode())
                            || ((index2ndView == m2ndViewReadMoreIndex)
    ...
    private void setMixedDisplayIndex(int displayMode) {
        ...
        mMixedDecoEmojiIndexList1stSplitQwerty.clear();
        mMixedEmojiIndexList1stSplitQwerty.clear();
        ...
        case DefaultSoftKeyboard.LANDSCAPE:
        default:
            //1stView DecoEmoji when Split Qwerty keyboard
            tempIndex2 = NUMBER_OF_ROW_NUM_LAND_DEFAULT * 2;
            for (int page = 0; page < NUMBER_OF_1ST_VIEW_SCROLL_MAX_PAGE_SPLIT_QWERTY; page++) {
                tempIndex1 = mScrollPageChangeNum1stSplitQwerty * page;
                for (int i = 0; i < (mCandidate1stViewColumnNumSplitQwerty / 2); i++) {
                    for (int j = 0; j < mScrollPageRowSize1stSplitQwerty[page]; j++) {
                        mMixedDecoEmojiIndexList1stSplitQwerty
                                .add((j + tempIndex1)
                                        + (i * tempIndex2));
                    }
                }
            ...
            //1stView DecoEmoji
            for (int page = 0; page < NUMBER_OF_1ST_VIEW_SCROLL_MAX_PAGE; page++) {
                tempIndex1 = mScrollPageChangeNum1st * page;
                if (page != (NUMBER_OF_1ST_VIEW_SCROLL_MAX_PAGE - 1)) {
                    // set index except last page
                    for (int i = 0; i < (mCandidate1stViewColumnNum / 2); i++) {
                        for (int j = 0; j < mScrollPageRowSize1st[page]; j++) {
                    ...
                    // rule is special
                    for (int j = 0; j < (mCandidate1stViewColumnNum / 2); j++) {
            ...
            //1stView Emoji when Split Qwerty keyboard
            for (int page = 0; page < NUMBER_OF_1ST_VIEW_SCROLL_MAX_PAGE_SPLIT_QWERTY; page++) {
                tempIndex1 = mScrollPageRowSize1stSplitQwerty[page]
                        + mScrollPageChangeNum1stSplitQwerty * page;
                for (int i = 0; i < (mCandidate1stViewColumnNumSplitQwerty / 2); i++) {
                    for (int j = 0; j < mScrollPageRowSize1stSplitQwerty[page]; j++) {
                        mMixedEmojiIndexList1stSplitQwerty.add((j + tempIndex1)
                                + (i * tempIndex2));
                    }
                }
            }
            ...
            //1stView Emoji
            for (int page = 0; page < NUMBER_OF_1ST_VIEW_SCROLL_MAX_PAGE; page++) {
                tempIndex1 = mScrollPageRowSize1st[page]
                        + mScrollPageChangeNum1st * page;
                if (page != (NUMBER_OF_1ST_VIEW_SCROLL_MAX_PAGE - 1)) {
                    // set index except last page
                    for (int i = 0; i < (mCandidate1stViewColumnNum / 2); i++) {
                        for (int j = 0; j < mScrollPageRowSize1st[page]; j++) {
    ...
    public void onDestroy() {
        EmojiAssist.getInstance().clearView();
    }
}
