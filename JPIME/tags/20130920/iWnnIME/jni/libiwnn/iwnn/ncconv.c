...
static NJ_INT16 conv_replaceSentence(NJC_CONV *x) {
    ...
    if (sameID < 0) {
        if ((new_sent->yomiLen != 0) &&
            (NJ_GET_EXT_YLEN_FROM_STEM(&new_sent->phrases[new_sent->phrNum-1].word) != 0) &&
            ((NJ_GET_EXT_YLEN_FROM_STEM(&new_sent->phrases[new_sent->phrNum-1].word) + NJ_GET_YLEN_FROM_FZK(&new_sent->phrases[new_sent->phrNum-1].word)) > NJ_MAX_LEN)) {
            /* 拡張検索で最大読み文字列長を超えている場合、追加しない */
        } else {
        /* 等価文が無ければ，新規追加 */
        x->sentNum++;
    ...
    if (x->cond.mode == ZEN_MODE_LEARN_PROC) {  /* 全候補解析 辞書検索 */
        ...
        while (empty == 0) {
            ...
            for (i = 0; i < x->fzkPhrNum; i++, fzk++) {
                ...
                /* 拡張検索時の読み文字列長を取得する */
                if ((NJ_GET_EXT_YLEN_FROM_STEM(&cand->word) + FZK_GET_YLEN(fzk)) > NJ_MAX_LEN) {
                    /* 最大読み文字列長 超過 */
                    continue;   /* 無視する */
                }
            ...
            while ((ret = njd_get_word(iwnn, pl->cur, &result, 1)) > 0) {
                ...
                /* 拡張検索時の読み文字列長を取得する */
                if ((NJ_GET_EXT_YLEN_FROM_STEM(&result.word) + FZK_GET_YLEN(fzk)) > NJ_MAX_LEN) {
                    /* 最大読み文字列長 超過 */
                    continue;   /* 無視する */
                }
