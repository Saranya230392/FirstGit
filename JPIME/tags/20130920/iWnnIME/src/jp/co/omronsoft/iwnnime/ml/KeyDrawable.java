...
public class KeyDrawable extends Drawable {
    ...
    private void drawKeyModekey(Resources r, Canvas canvas) {
        ...
        if (keyboard.getKeyboardType() == DefaultSoftKeyboard.KEYBOARD_12KEY) {
            if (isFloatingShown) {
                // TODO QA No.34 text size unknown
            ...
            if (isFloatingShown) {
                ...
                if (!mOneHandedShown ) {
                    ...
                    // TODO QA No.37 text size unknown
        ...
        final int drawableX = - (icon.getIntrinsicWidth() - mKeyWidth / 2) - subIconGapX;
...
//                positionY = (float)(textBlockSize03 - mKeyHeight / 2 +
//                        OpenWnn.getContext().getResources().getDimensionPixelSize(R.dimen.key_onehanded_mode_qwert_moji_text_top));
    ...
    private void drawChangeLanguagePopkey(Resources r, Canvas canvas) {
        ...
        } else if ((DefaultSoftKeyboard.mCurrentKeyMode == DefaultSoftKeyboard.KEYMODE_JA_FULL_ALPHABET
                 || DefaultSoftKeyboard.mCurrentKeyMode == DefaultSoftKeyboard.KEYMODE_JA_HALF_ALPHABET)
                ) {
        ...
        if (mId == KEY_ID_CHANGE_LANG_TITLE) {
            if (isPressed) {
                if (!mKeySkin.isValid()) {
                    icon = r.getDrawable(R.drawable.ime_keypad_icon_popup_setting_pressed);
                } else {
                    icon = mKeySkin.getDrawable(R.drawable.ime_keypad_icon_popup_setting_pressed);
                }
            } else {
                if (!mKeySkin.isValid()) {
                    icon = r.getDrawable(R.drawable.ime_keypad_icon_popup_setting_normal);
                } else {
                    icon = mKeySkin.getDrawable(R.drawable.ime_keypad_icon_popup_setting_normal);
                }
            }
        ...
        if (icon != null) {
            positionX = r.getDimensionPixelSize(R.dimen.popup_change_language_width)
                    ...
                    - r.getDimensionPixelSize(R.dimen.popup_view_padding_change_language_right);
            if (mId == KEY_ID_CHANGE_LANG_TITLE) {
                positionX += r.getDimensionPixelSize(R.dimen.popup_change_language_horizontal_gap);
            }
    ...
    private void drawLeftkey(Resources r, Canvas canvas) {
        ...
        final int drawableSubIconX = - (subIcon.getIntrinsicWidth() - mKeyWidth / 2) - subIconGapX;
    ...
    private void drawFunctionkey(Resources r, Canvas canvas) {
        ...
        boolean isQwerty = (keyboard.getKeyboardType() == DefaultSoftKeyboard.KEYBOARD_QWERTY);
        boolean isSmall = false;
        if ((mOneTouchEmojiShown || mOneHandedShown) && mVerticalDisp && isQwerty) {
            isSmall = true;
        }
        ...
        final int drawableSubIconX = -(subIcon.getIntrinsicWidth() - mKeyWidth / 2) - adjustX;
    ...
    private void drawSymkey(Resources r, Canvas canvas) {
        ...
        if(!mKeySkin.isValid()){
            if (isQwerty) {
                if (mOneTouchEmojiShown || isFloatingShown) {
            ...
            slashIcon = r.getDrawable(R.drawable.ime_keypad_icon_slash);
            subIcon = r.getDrawable(R.drawable.ime_keypad_btn_more);
        }else{
            if (isQwerty) {
                if (mOneTouchEmojiShown || isFloatingShown) {
            ...
            subIcon = mKeySkin.getDrawable(R.drawable.ime_keypad_btn_more);
        }
        ...
        if (keyboard.getKeyboardType() == DefaultSoftKeyboard.KEYBOARD_QWERTY) {
            ...
            if (isFloatingShown) {
                ...
                if (!mOneHandedShown) {
                    ...
                    mTextBlock01Size = r.getDimensionPixelSize(R.dimen.key_mode_on_text_small_size);
                    drawableX =  drawableSlashX
                                    + r.getDimensionPixelSize(R.dimen.key_onehanded_sym_12key_sym_left);
                    drawableY =  - (mKeyHeight / 2) + r.getDimensionPixelSize(R.dimen.key_onehanded_sym_12key_sym_top);
                }
            } else {
                mTextBlock01Size = r.getDimensionPixelSize(R.dimen.key_mode_on_text_small_size);
                drawableX =  drawableSlashX  + slashIcon.getIntrinsicWidth()
        ...
        if (keyboard.getKeyboardType() == DefaultSoftKeyboard.KEYBOARD_QWERTY) {
            ...
            if (isFloatingShown) {
                ...
                if (!mOneHandedShown) {
                    ...
                    positionX = drawableSlashX + r.getDimensionPixelSize(R.dimen.key_onehanded_sym_12key_num_left);
                    positionY = - (mKeyHeight / 2) + r.getDimensionPixelSize(R.dimen.key_onehanded_sym_12key_num_top)
                            + mTextBlock01Size;
                }
            } else {
        ...
        if( KEY_ID_DAKUTEN != mId){
            ...
            if (TEXTBLOCK_01[mId - 1] != null) {
                ...
                if (mVerticalDisp) {
                    ...
                    positionX -= (float)(Math.abs(fontMetrics.descent + fontMetrics.ascent)*2.9);
                }
    ...
    private void drawNavikey(Resources r, Canvas canvas) {
        ...
        if ((mOneHandedShown || isFloatingShown) || WnnKeyboardFactory.getSplitMode()) {
    ...
    private void drawFlickPopupKeySymkey(Resources r, Canvas canvas) {
        ...
        float right_gap = OpenWnn.getContext().getResources().getDimensionPixelSize(R.dimen.key_mode_change_right_gap);
        float keyPadding = OpenWnn.getContext().getResources().getDimensionPixelSize(R.dimen.key_mode_change_key_padding);
        float oneKeyWidth = (mKeyWidth - left_gap - right_gap) / 2;
        ...
        case KEY_MODE_CHANGE_SYMNUM_NUM_B_PREVIEW:
            paint.setColor(mKeySkin.getColor(OpenWnn.getContext(), R.color.key_mode_change_bg_color));
            paint.setStyle(Paint.Style.FILL);
            canvas.drawRect(positionX - (oneKeyWidth / 2) + (keyPadding / 2),
                            positionY*2,
                            positionX + (oneKeyWidth / 2) - (keyPadding / 2),
                            -positionY,
                            paint);
        ...
        case KEY_MODE_CHANGE_SYMNUM_SYM_B_PREVIEW:
            paint.setColor(mKeySkin.getColor(OpenWnn.getContext(), R.color.key_mode_change_bg_color));
            paint.setStyle(Paint.Style.FILL);
            canvas.drawRect(positionX - (oneKeyWidth / 2) + oneKeyWidth + (keyPadding / 2),
                            positionY*2,
                            positionX + (oneKeyWidth / 2) + oneKeyWidth - (keyPadding / 2),
                            -positionY,
                            paint);
            mPaintTextBlock02.setColor(mKeySkin.getColor(OpenWnn.getContext(), R.color.key_mode_change_text_pushed_color));
            break;
        ...
        canvas.drawText("12?", positionX, 0, mPaintTextBlock01);
        ...
        positionX += (oneKeyWidth - (icon.getIntrinsicWidth() / 2));
        positionY = - (icon.getIntrinsicHeight() * 4 / 6);
        canvas.translate(positionX, positionY);
        icon.setBounds(0, 0, icon.getIntrinsicWidth(), icon.getIntrinsicHeight());
        icon.draw(canvas);
        canvas.translate(-positionX, -positionY);
        ...
        if( KEY_ID_DAKUTEN != offsetId){
            ...
            if (TEXTBLOCK_04[offsetId - 1] != null) {
                ...
                if (mVerticalDisp) {
                    ...
                    positionX += (float)(Math.abs(fontMetrics.descent + fontMetrics.ascent)*4);
                }
    ...
    public void draw(Canvas canvas) {
        ...
        try {
            ...
            } else if (mId >= KEY_ID_CHANGE_LANG_TITLE && mId <= KEY_ID_CHANGE_LANG_EN) {
