...
import com.android.settings.TetherNetworkSettings; // yonguk.kim JB+ migration 20130130
...
public class PowerUsageDetail extends Fragment implements Button.OnClickListener {
    ...
    private static final String LG_MLT = "com.lge.mlt";
    ...
    private void startApplicationDetailsActivity() {
        ...
        String[] packages = mPm.getPackagesForUid(mUid);

        if (packages == null) {
            return;
        }

        boolean isAndorid = false;
        ...
        Log.d(TAG, "isAndorid: " + isAndorid);

        if (isAndorid) {
            args.putString(InstalledAppDetails.ARG_PACKAGE_NAME, "android");
            ...
            if (getActivity() instanceof PreferenceActivity) {
                ...
                Intent intent = com.android.settings.Utils.buildStartFragmentIntent(
                        InstalledAppDetails.class.getName(), args, R.string.application_info_label,
                        0, getActivity(), R.drawable.shortcut_apps);
                startActivity(intent);
            }
        } else {
            args.putString(InstalledAppDetails.ARG_PACKAGE_NAME, packages[0]);
            if (getActivity() instanceof PreferenceActivity) {
                PreferenceActivity pa = (PreferenceActivity)getActivity();
                pa.startPreferencePanel(InstalledAppDetails.class.getName(), args,
                        R.string.application_info_label, null, null, 0);
    ...
    private void doAction(int action) {
        if ((getActivity() instanceof PreferenceActivity) == false) {
            doActionForUsageManager(action);
            return;
        }
        ...
        case ACTION_DISPLAY_SETTINGS:
            pa.startPreferencePanel(DisplaySettings.class.getName(), null,
                    ...
                    R.string.display_settings, null, null, 0);
            break;
        ...
        case ACTION_WIRELESS_SETTINGS:
            if (Config.supportAirplaneListMenu() && Utils.isTablet()) {
                pa.startPreferencePanel(AirplaneModeFragment.class.getName(), null,
                        R.string.airplane_mode, null, null, 0);
            } else if (!(com.android.settings.Utils.isUpgradeModel()
                    || com.android.settings.Utils.isVeeModel(getActivity())
                    || "DCM".equals(Config.getOperator()))
                    || ("ATT".equals(Config.getOperator()) && "geeb".equals(Build.DEVICE))) {
                pa.startPreferencePanel(TetherNetworkSettings.class.getName(), null,
                        R.string.sp_title_tether_network_NORMAL_jb_plus, null, null, 0);
            } else { // yonguk.kim JB+ migration
    ...
    // 20131113 yonguk.kim Apply UsageManager for ATT
    private void doActionForUsageManager(int action) {
        Activity activity = getActivity();
        Intent intent = null;
        switch (action) {
        case ACTION_DISPLAY_SETTINGS:
            intent = com.android.settings.Utils.buildStartFragmentIntent(
                    DisplaySettings.class.getName(), null,
                    R.string.display_settings, 0, activity, R.drawable.shortcut_display);
            break;
        case ACTION_WIFI_SETTINGS:
        ...
        case ACTION_WIRELESS_SETTINGS:
            if (Config.supportAirplaneListMenu() && Utils.isTablet()) {
                intent = com.android.settings.Utils.buildStartFragmentIntent(
                        AirplaneModeFragment.class.getName(), null,
                        R.string.airplane_mode, 0, activity, 0);
                    ...
                    || ("ATT".equals(Config.getOperator()) && "geeb".equals(Build.DEVICE))) {
                intent = com.android.settings.Utils.buildStartFragmentIntent(
                        TetherNetworkSettings.class.getName(), null,
                        R.string.sp_title_tether_network_NORMAL_jb_plus,
                        0, activity, R.drawable.shortcut_networks_setting);
            } else { // yonguk.kim JB+ migration
                intent = com.android.settings.Utils.buildStartFragmentIntent(
                        WirelessSettings.class.getName(), null,
                        R.string.radio_controls_title, 0, activity, 0);
            }
        ...
        case ACTION_LOCATION_SETTINGS:
            intent = com.android.settings.Utils.buildStartFragmentIntent(
                    ...
                    R.string.location_settings_title, 0, activity, R.drawable.shortcut_location);
            break;
        case ACTION_FORCE_STOP:
            killProcesses();
            break;
        ...
        if (mTypes != null && mValues != null) {
            for (int i = 0; i < mTypes.length; i++) {
                ...
                case R.string.usage_type_no_coverage:
                    if (com.android.settings.Utils.isRTLLanguage()) {
    ...
    private void fillControlsSection(int uid) {
        ...
        case APP:
            ...
            break;
        case SCREEN:
            addControl(R.string.battery_display_settings,
                    R.string.battery_sugg_display,
                    ACTION_DISPLAY_SETTINGS);
            removeHeader = false;
            break;
        case WIFI:
            addControl(R.string.battery_wifi_settings,
    ...
    private void fillPackagesSection(int uid) {
        ...
        for (int i = 0; i < mPackages.length; i++) {
            try {
                ...
                if (!LG_MLT.equals(ai.packageName)) {
    ...
    private String getDescriptionForDrainType() {
        ...
        return mDrainType != null ? getResources().getString(
                sDrainTypeDesciptions[mDrainType.ordinal()]) : "";
    }
}
