...
public class StorageMeasurement {
    ...
    private static final int REC_MARGIN_SIZE = 1024 * 1024;
    ...
    private void sendInternalApproximateUpdate() {
        ...
        receiver.updateApproximate(this, mTotalSize, mAvailSize, mReservedSize);
    }
    ...
    private class MeasurementHandler extends Handler {
        ...
        private void measureApproximateStorage(IMediaContainerService imcs) {
            ...
            try {
                final StatFs stat = new StatFs(mVolume != null
                        ? mVolume.getPath() : Environment.getDataDirectory().getPath());
                final long blockSize = stat.getBlockSize();
                final long totalBlocks = stat.getBlockCount();
                final long availableBlocks = stat.getAvailableBlocks();
                final long freeBlocks = stat.getFreeBlocks();

                mTotalSize = totalBlocks * blockSize;
                mAvailSize = availableBlocks * blockSize;
                mReservedSize = (freeBlocks - availableBlocks) * blockSize;
            } catch (IllegalArgumentException e) {
                Log.e(TAG, "Fail to access external storage");
                return;
            }
        ...
        private void measureExactStorage(IMediaContainerService imcs) {
            ...
            details.reservedSize = mReservedSize;
            ...
            // [seungyeop.yeom][2014-05-12] display capacity of TV app
            if (Utils.checkPackage(context, "com.lge.oneseg")) {
                File path = new File("/data/tvbox");
                long mUsedSize;
                try {
                    mUsedSize = (long)(getFolderSize(path));
                } catch (Exception e) {
                    Log.e("YSY", "Fail to access path of TV app");
                    mUsedSize = 0;
                }
                Log.d("YSY", "TV Box size : " + mUsedSize);
                details.tvAppsSize = mUsedSize;
            }
    ...
    static class FileInfo implements Comparable<FileInfo> {
        ...
        // [seungyeop.yeom][2013-11-26] Declare the method to know the file name.
    ...
    // [START][seungyeop.yeom][2014-05-12] display capacity of TV app
    public static long getFolderSize(File dir) {
        long size = 0;
        if (dir.exists() == false || dir.isDirectory() == false) {
            Log.e("YSY", "getFolderSize() dir" + dir + " is not exists or not a directory.");
            return size;
        }
        ...
        if (size > REC_MARGIN_SIZE) {
            size = size - REC_MARGIN_SIZE;
        } else {
            size = 0;
        }
