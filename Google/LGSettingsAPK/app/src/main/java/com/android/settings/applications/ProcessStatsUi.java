...
public class ProcessStatsUi extends PreferenceFragment
    ...
    private void refreshStats() {
        ...
        */
        ...
        if (DEBUG) {
            Log.d(TAG, "Total time of stats: " + makeDuration(mTotalTime));
        }
        ...
        */
                ...
                for (int iv = 0; iv < vpkgs.size(); iv++) {
                    ...
                    for (int iproc = 0; iproc < st.mProcesses.size(); iproc++) {
                        ...
                        if (ent == null) {
                            ...
                            if (ent.mDuration > 0) {
                                if (DEBUG) {
                                    Log.d(TAG, "Adding proc " + proc.mName + "/"
        ...
        if (mStatsType == MENU_TYPE_BACKGROUND) {
            for (int ip = 0, n = mStats.mPackages.getMap().size(); ip < n; ip++) {
                SparseArray<SparseArray<ProcessStats.PackageState>> uids = mStats.mPackages
                        .getMap().valueAt(ip);
                    ...
                    for (int iv = 0; iv < vpkgs.size(); iv++) {
                        ...
                        for (int is = 0, ns = ps.mServices.size(); is < ns; is++) {
                            ...
                            if (ss.mProcessName != null) {
                                ProcStatsEntry ent = entriesMap
                                        .get(ss.mProcessName, uids.keyAt(iu));
                                if (ent != null) {
                                    if (DEBUG) {
                                        Log.d(TAG, "Adding service " + ps.mPackageName
                                                + "/" + ss.mName + "/" + uids.keyAt(iu)
                                                + " to proc "
                                                + ss.mProcessName);
                                    }
                                    ent.addService(ss);
                                    ...
                                    Log.w(TAG,
                                            "No process " + ss.mProcessName + "/" + uids.keyAt(iu)
        ...
        for (int i = 0; i <= end; i++) {
            ...
            if (mAppListGroup.getPreferenceCount() > (MAX_ITEMS_TO_LIST + 1)) {
                if (DEBUG) {
                    Log.d(TAG, "Done with UI, hit item limit!");
                }
                break;
                // Add in service info.
                //ProcessStats.ServiceState
            }
