...
public class AppWidgetPickActivity extends ActivityPicker
    ...
    protected List<PickAdapter.Item> getItems() {
        List<PickAdapter.Item> items = mAppWidgetLoader.getItems(getIntent());
        mItems = new ArrayList<PickAdapter.Item>();

        for (PickAdapter.Item item : items) {
            if (item == null || item.packageName == null) {
                continue;	
            }
            // Remove LG Widget in 3-rd Party Home requested by Widget Part : g1.kim@lge.com
            if (item.packageName.startsWith("com.lge") || "com.android.calendar".equals(item.packageName)) {
                continue;
            }
            mItems.add(item);
        }
    ...
    public PickAdapter.Item createItem(Context context, AppWidgetProviderInfo info, Bundle extras) {
        ...
        if (info.icon != 0) {
            try {
                ...
                if (mWantNoThemeIcon) {
                    icon = loadNoThemeIcon(mPackageManager, info.provider.getPackageName(), null, info.icon);
                } else {
            ...
            if (icon == null) {
			    ...
			    item = new PickAdapter.Item(context, label, icon);
