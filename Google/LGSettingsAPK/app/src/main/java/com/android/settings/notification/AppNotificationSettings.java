...
public class AppNotificationSettings extends SettingsPreferenceFragment {
    ...
    private CheckBoxPreference mBlock;
    private CheckBoxPreference mPriority;
    private CheckBoxPreference mSensitive;
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if (DEBUG) {
            Log.d(TAG, "onActivityCreated mCreated=" + mCreated);
        }
        if (mCreated) {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (Utils.supportSplitView(getActivity())) {
            ...
            intent.putExtra(Settings.EXTRA_APP_PACKAGE,
                    bundle.getString(Settings.EXTRA_APP_PACKAGE))
                    .putExtra(Settings.EXTRA_APP_UID,
                            bundle.getInt(Settings.EXTRA_APP_UID))
                    .putExtra(EXTRA_HAS_SETTINGS_INTENT,
                            bundle.getBoolean(EXTRA_HAS_SETTINGS_INTENT))
                    .putExtra(EXTRA_SETTINGS_INTENT,
                            bundle.getParcelable(EXTRA_SETTINGS_INTENT));
        } else {
        ...
        if (DEBUG) {
            Log.d(TAG, "Load details for pkg=" + pkg + " uid=" + uid);
        }
        ...
        mBlock = (CheckBoxPreference)findPreference(KEY_BLOCK);
        mPriority = (CheckBoxPreference)findPreference(KEY_PRIORITY);
        mSensitive = (CheckBoxPreference)findPreference(KEY_SENSITIVE);
        ...
        if (Utils.supportSplitView(getActivity())) {
            SettingsBreadCrumb breadCrumb = SettingsBreadCrumb
                    .get(getActivity());
            if (breadCrumb != null) {
                breadCrumb.setTitle(mAppRow.label.toString());
            }
        } else {
            actionBar.setTitle(mAppRow.label);
        }
        ...
        if (Utils.getsoundAppsNoneList(mContext).contains(pkg)) {
            Log.d(TAG, "blocked pkg : " + pkg);
            getPreferenceScreen().removePreference(mBlock);
            // don't have it depend on a preference that's gone
            mPriority.setDependency(null);
        }
    }

    private void getSensitiveEanbled(boolean mBlock) {
        if (mSensitive != null) {
            if (mBlock) {
                mSensitive.setEnabled(false);
            } else {
                mSensitive.setEnabled(true);
            }
        }
    }
...
class SettingsNotiAppSwitchPreference extends SwitchPreference {
    ...
    public SettingsNotiAppSwitchPreference(Context context, AttributeSet attrs) {
        super(context, attrs);
        setWidgetLayoutResource(R.layout.preference_widget_switch_app_noti);
    }

    public SettingsNotiAppSwitchPreference(Context context) {
        this(context, null);
    }
