...
import com.android.settings.LocationCustomPreference;
    ...
    public List<Preference> getAppList() {
        ...
        for (int i = 0; i < appOpsN; ++i) {
            ...
            boolean isAndroidOs = (uid == Process.SYSTEM_UID)
                    && ANDROID_SYSTEM_PACKAGE_NAME.equals(packageName);
...
//            Resources res = mActivity.getResources(); // Native Code MD Error 

            final UserHandle userHandle = new UserHandle(userId);
            Drawable appIcon = mPackageManager.getApplicationIcon(appInfo);
            Drawable icon = um.getBadgedIconForUser(appIcon, userHandle);
