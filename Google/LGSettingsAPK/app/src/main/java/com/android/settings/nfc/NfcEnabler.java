...
public class NfcEnabler implements Preference.OnPreferenceChangeListener {
    ...
    private final static String TAG = "NfcSetting";
    ...
    public void resume() {
        ...
        if (com.android.settings.lgesetting.Config.Config.THREELM_MDM) {
            ...
            // Check if error was due to 3LM disabling NFC
            if (SystemProperties.getInt("persist.security.nfc.lockout", 1) == 0) {
                mCheckbox.setEnabled(false);
                mCheckbox.setSummary(R.string.nfc_toggle_disabled);
            }
        }
    ...
    public boolean onPreferenceChange(Preference preference, Object value) {
        ...
        if (desiredState) {
            ...
            // Intent(NfcAdapter.ACTION_ADAPTER_STATE_CHANGED)); // kerry
        } else {
            mNfcAdapter.disable();
            // mContext.sendBroadcast(new
            // Intent(NfcAdapter.ACTION_ADAPTER_STATE_CHANGED)); // kerry
    ...
    private void handleNfcStateChanged(int newState) {
        ...
        case NfcAdapter.STATE_TURNING_ON:
            Log.d(TAG, "NfcAdapter.STATE_TURNING_ON");
            mCheckbox.setChecked(true);
            mCheckbox.setEnabled(false);
            mAndroidBeam.setEnabled(false);
            break;
        case NfcAdapter.STATE_TURNING_OFF:
            Log.d(TAG, "NfcAdapter.STATE_TURNING_OFF");
            mCheckbox.setChecked(false);
            mCheckbox.setEnabled(false);
            mAndroidBeam.setEnabled(false);
            break;
