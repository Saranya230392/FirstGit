...
public class TextToSpeechSettings extends SettingsPreferenceFragment implements
    ...
    private static final String KEY_RATE_LIST = "tts_rate_list";

    private static final String KEY_PITCH_LIST = "tts_pitch_list";
    ...
    private Preference mTtsRatePref;
    private Preference mTtsPitchPref;
    ...
    //[S] [2012.02.20][juncheol.han@lge.com] AudioFocus setting
    private AudioManager mAudioManager;
    //[E] [2012.02.20][juncheol.han@lge.com] AudioFocus setting
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // [S][2011.12.08][hyoungjun21.lee@lge.com][Common] Changed activity title
        getActivity().setTitle(R.string.tts_settings_title);
        // [E][2011.12.08][hyoungjun21.lee@lge.com][Common] Changed activity title
        ...
        mTtsRatePref = (Preference)findPreference(KEY_RATE_LIST);
        mTtsPitchPref = (Preference)findPreference(KEY_PITCH_LIST);
        ...
        //[S] [2012.02.20][juncheol.han@lge.com] AudioFocus setting
        mAudioManager = (AudioManager)getSystemService(getActivity().getApplicationContext().AUDIO_SERVICE);
        //[E] [2012.02.20][juncheol.han@lge.com] AudioFocus setting
    ...
    public void onResume() {
        ...
        mDefaultPitch = Settings.Secure.getInt(resolver,
                Settings.Secure.TTS_DEFAULT_PITCH,
                TextToSpeech.Engine.DEFAULT_PITCH);

        String[] mstr = getResources().getStringArray(R.array.tts_rate_values);
        String[] metry = getResources().getStringArray(R.array.tts_rate_entries);

        for (int i = 0; i < mstr.length; i++) {
            if (String.valueOf(mDefaultRate).equals(mstr[i])) {
                mTtsRatePref.setSummary(metry[i]);
            }
        }
        ...
        String[] mPitchValue = getResources().getStringArray(arrayId);
        String[] mPitchEntries = getResources().getStringArray(
                R.array.tts_pitch_entries);
        for (int i = 0; i < mPitchValue.length; i++) {
            if (String.valueOf(mDefaultPitch).equals(mPitchValue[i])) {
                mTtsPitchPref.setSummary(mPitchEntries[i]);
            }
        }
    ...
    private void setTtsUtteranceProgressListener() {
        ...
        mTts.setOnUtteranceProgressListener(new UtteranceProgressListener() {
            ...
            @Override
            public void onDone(String utteranceId) { /* empty */
            }
        ...
        try {
            ...
            for (String loc : mAvailableStrLocals) {
               if (mCurrentEngine.equals(LG_TTS)
                     ...
                     if (loc.contains(defaultLocaleStr)) {
    ...
    private void onSampleTextReceived(int resultCode, Intent data) {
        ...
        if (resultCode == TextToSpeech.LANG_AVAILABLE && data != null) {
            Log.d(TAG, "data: " + data);
            Log.d(TAG, "data.getStringExtra" + data.getStringExtra("sampleText"));
    ...
    private void updateWidgetState(boolean enable) {
        ...
        mTtsRatePref.setEnabled(enable);
        mTtsPitchPref.setEnabled(enable);
    ...
    private void updateEngineStatus(int resourceId) {
        ...
        String localeName = locale.getDisplayName().substring(0, 1).toUpperCase()
                + locale.getDisplayName().substring(1);

        Log.d(TAG , "(locale.getVariant()) :" + locale.getVariant());
        Log.d(TAG , "(locale.getLanguage()) :" + locale.getLanguage());
        Log.d(TAG , "(locale.getISO3Language()) :" + locale.getISO3Language());

            if ("FEMALE1".equals(locale.getVariant())) {
                if ("kor".equals(locale.getISO3Language())) {
                    localeName = getString(R.string.tts_lang_kor_female1);
                }
                if ("eng".equals(locale.getISO3Language())) {
                    localeName = getString(R.string.tts_lang_usa_female1);
                }
            } else if ("FEMALE2".equals(locale.getVariant())) {
                if ("kor".equals(locale.getISO3Language())) {
                    localeName = getString(R.string.tts_lang_kor_female2);
                }
                if ("eng".equals(locale.getISO3Language())) {
                    localeName = getString(R.string.tts_lang_usa_female2);
                }
            } else if ("MALE1".equals(locale.getVariant())) {
                if ("kor".equals(locale.getISO3Language())) {
                    localeName = getString(R.string.tts_lang_kor_male1);
                }
            } else {
                localeName = locale.getDisplayName().substring(0, 1).toUpperCase()
                    + locale.getDisplayName().substring(1);
            }
        mEngineStatus.setSummary(getString(resourceId, localeName));
    }
    ...
    private void onVoiceDataIntegrityCheckDone(Intent data) {
        ...
        Log.e(TAG, "onVoiceDataIntegrityCheckDone  = " + evaluateDefaultLocale());
    ...
    private void ttsInstallNotification() {
        if (Config.getOperator().equals(Config.VZW)) {
            Log.d(TAG, "operator VZW!!    AlertDialog show");
            new AlertDialog.Builder(getActivity())
                    .setTitle(R.string.sp_dlg_note_NORMAL)
                    .setMessage(R.string.sp_InstallVoiceData_NORMAL)
