...
public final class CredentialStorage extends Activity {
    ...
    // 3LM_MDM_START KK for 3LM SD encryption
    public static final String ACTION_RESET_3LM_KEYSTORE = "com.threelm.dm.intent.KEYSTORE_RESET";
    ...
    /* 2013-01-21 hyungsukim(swgi.kim@lge.com) [F240L/TD285458] It dismiss dialog-popup when changed screen */
    ...
    protected void onResume() {
        ...
        UserManager userManager = (UserManager)getSystemService(
                                  getApplicationContext().USER_SERVICE);
        if (!userManager.hasUserRestriction(UserManager.DISALLOW_CONFIG_CREDENTIALS)) {
            callingFrom = (String)intent.getExtra("CallingFrom");
            Log.d(TAG, "getCallingPackage() = " + callingFrom);
    ...
    private void handleUnlockOrInstall() {
        ...
        case UNLOCKED: {
            if (!checkKeyGuardQuality()) {
                new ConfigureKeyGuardDialog(callingFrom);
                return;
            }
            installIfAvailable();
            finish();
    ...
    private void installIfAvailable() {
        if (mInstallBundle == null) {
            return;
        } else {
            if (mInstallBundle.isEmpty()) {
                return;
            }
        }
        ...
        if (!checkKey(bundle, uid, flags)) {
            return;
        }

        if (!checkCNFeature(bundle)) {
            return;
        }
    ...
    private boolean checkKey(Bundle bundle, int uid, int flags) {
    ...
    private boolean checkCNFeature (Bundle bundle) {
        ...
        if (bundle.containsKey(/*Credentials.*/EXTRA_WAPI_AS_CERTIFICATES_NAME)) {
            ...
            if (!mKeyStore.put(caListName, caListData, Process.WIFI_UID, KeyStore.FLAG_NONE)) {
...
// WAPI
            ...
            implements DialogInterface.OnClickListener, DialogInterface.OnDismissListener
    {
        ...
        private ResetDialog() {
            ...
            if (Utils.isUI_4_1_model(getApplicationContext()) || "CN".equals(Config.getCountry())) {
            } else {
    ...
    private class ResetKeyStoreAndKeyChain extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... unused) {
            ...
            // Remove 3LM keys if needed
            if (com.android.settings.lgesetting.Config.Config.THREELM_MDM) {
                if (Settings.Secure.getInt(getContentResolver(), "boot_lock", 0) == 1) {
                    Intent intent = new Intent();
                    intent.setAction(ACTION_RESET_3LM_KEYSTORE);
                    intent.putExtra(EXTRA_RESET, 1);
            ...
            implements DialogInterface.OnClickListener, DialogInterface.OnDismissListener
    {
        ...
        private ConfigureKeyGuardDialog(String fromPackage) {
            int message = R.string.sp_credentials_configure_lock_screen_hint_NORMAL;
            if (fromPackage != null && fromPackage.equals("com.android.settings.vpn2")) {
                message = R.string.sp_credentials_configure_lock_screen_hint_NORMAL_vpn;
            }
            AlertDialog dialog = new AlertDialog.Builder(CredentialStorage.this)
                    .setTitle(R.string.sp_dlg_note_NORMAL)
                    // .setIconAttribute(android.R.attr.alertDialogIcon)
                    .setMessage(message)
                    .setPositiveButton(R.string.sp_settings_label_NORMAL, this)
