...
public class NotificationAccessSettings extends ListFragment {
    ...
    public static final String[] HIDE_ITEM = {
        "com.lge.lockscreen.quickCover.LgeQuickCoverNLService"
    };
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (actionBar != null && !Utils.supportSplitView(getActivity())) {
    ...
    private static int getListeners(ArrayAdapter<ServiceInfo> adapter, PackageManager pm) {
        ...
        for (int i = 0, count = installedServices.size(); i < count; i++) {
            ...
            if (adapter != null) {
                Log.i(TAG, "[getListeners]package : " + info.name);

                for (int j = 0; j < HIDE_ITEM.length; j++) {
                    if (HIDE_ITEM[j].equals(info.name)) {
                    }
                    else {
                        adapter.add(info);
                    }
