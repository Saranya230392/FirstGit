...
/* LGE_CHANGE_S, wooseong.park@lge.com, 2012-08-08, <Anshinmanager check box deactivate method synchronized with DeviceManager> */
import android.os.Handler;
...
/* LGE_CHANGE_E, wooseong.park@lge.com, 2012-08-08, <Anshinmanager check box deactivate method synchronized with DeviceManager> */
...
public class DeviceAdminAdd extends Activity {
    ...
    protected void onCreate(Bundle icicle) {
        ...
        try {
            ...
            // LGMDM [a1-mdm-dev@lge.com]
            if (com.lge.config.ConfigBuildFlags.CAPP_MDM) {
                com.android.settings.MDMSettingsAdapter.getInstance().createLGMDMDeviceAdminInfo(this, ri);
            }
            // LGMDM_END
        ...
        if (DevicePolicyManager.ACTION_ADD_DEVICE_ADMIN.equals(getIntent().getAction())) {
            ...
            if (mDPM.isAdminActive(cn)) {
                ...
                // LGMDM [a1-mdm-dev@lge.com]
                if (!mRefreshing && com.lge.config.ConfigBuildFlags.CAPP_MDM) {
                    mRefreshing = com.android.settings.MDMSettingsAdapter.getInstance().
                            checkLGMDMUsePolicies(cn);
                }
                // LGMDM_END
        ...
        switch (id) {
            case DIALOG_WARNING: {
                ...
                AlertDialog.Builder builder = new AlertDialog.Builder(
                        DeviceAdminAdd.this);
                builder.setTitle(R.string.sp_warning_NORMAL);
                builder.setMessage(msg);
                builder.setPositiveButton(R.string.dlg_ok,
                        new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        mDPM.removeActiveAdmin(mDeviceAdmin.getComponent());
                        //[S][20130906][TD:87232][swgi.kim@lge.com]
                        //Timing issue. When dialog dismiss it does not update list
                        // finish();
                        handler.sendEmptyMessageDelayed(0, DELAY_TIME);
                    }
                });
    ...
    void updateInterface() {
        ...
        if (!mRefreshing && mDPM.isAdminActive(mDeviceAdmin.getComponent())) {
            if (mActivePolicies.size() == 0) {
                ...
                // GMDM [a1-mdm-dev@lge.com]
                if (com.lge.config.ConfigBuildFlags.CAPP_MDM) {
                    com.android.settings.MDMSettingsAdapter.getInstance()
                            .addLGMDMDeivceAdminPolicyInfo(policies);
                }
                // LGMDM_END
...
/* LGE_CHANGE_S, wooseong.park@lge.com, 2012-08-08, <Anshinmanager check box deactivate method synchronized with DeviceManager> */
    ...
    void createThreadAndDialog() {
        while ( true ) {
            ...
            if (cur != null) {
                ...
                if (!mActiveAdmins.contains(mDeviceAdmin.getComponent())) {
                    Log.w(TAG, "anmane not find! : mFlagBtnClick =  " + mFlagBtnClick);
                    break;
                }
           }
    ...
    private Handler handler = new Handler() {
        public void handleMessage(Message msg) {
            Log.w("parws", "handleMessage! : mFlagBtnClick =  " + mFlagBtnClick);
            finish();
        }
    };
