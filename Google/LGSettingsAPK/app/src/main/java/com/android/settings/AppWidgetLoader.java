...
public class AppWidgetLoader<Item extends AppWidgetLoader.LabelledItem> {
    ...
    void putAppWidgetItems(List<AppWidgetProviderInfo> appWidgets,
        ...
        List<String> excludedWidgetListPack = getExcludedWidgetList(mContext,
                "exclude_widget_from_lockscreen_package");
        List<String> excludedWidgetListClass = getExcludedWidgetList(mContext,
                "exclude_widget_from_lockscreen_class");
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (excludedWidgetListPack != null && excludedWidgetListClass != null) {
                int indexCls = excludedWidgetListClass.indexOf(info.provider.getClassName());
                int indexPkg = excludedWidgetListPack.indexOf(info.provider.getPackageName());
                if (indexPkg != -1 && indexPkg == indexCls) {
                    continue;
                }
            }
    ...
    private List<String> getExcludedWidgetList(Context context, String arrayname) {
        String[] quickWindowWidgets = LockSettingsFeatureConfig
                .getStringArray(context, arrayname);
        if (quickWindowWidgets != null) {
            return Arrays.asList(quickWindowWidgets);
        }
        return null;
    }
