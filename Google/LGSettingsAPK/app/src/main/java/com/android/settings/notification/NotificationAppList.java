...
public class NotificationAppList extends PinnedHeaderListFragment
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (Utils.supportSplitView(getActivity())) {
            ...
            if (breadCrumb != null) {
                breadCrumb
                        .setTitle(getString(R.string.app_notifications_title));
            }
        } else {
            if (actionBar != null) {
    ...
    private void repositionScrollbar() {
        ...
        if (DEBUG) {
        Log.d(TAG, String.format("Eating %dpx into %dpx padding for %dpx scroll, ld=%d",
            ...
            vh.row.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    if (Utils.supportSplitView(getActivity())) {
                        ...
                        bundle.putString(Settings.EXTRA_APP_PACKAGE, row.pkg);
                        bundle.putInt(Settings.EXTRA_APP_UID, row.uid);
                        bundle.putBoolean(EXTRA_HAS_SETTINGS_INTENT,
                                row.settingsIntent != null);
                        bundle.putParcelable(EXTRA_SETTINGS_INTENT,
                                row.settingsIntent);

                        PreferenceActivity preferenceActivity = (PreferenceActivity)getActivity();
                        preferenceActivity.startPreferencePanel(
                                AppNotificationSettings.class.getName(),
                                bundle, R.string.app_notifications_title, null,
                                null, 0);
    ...
    private final Runnable mCollectAppsRunnable = new Runnable() {
        ...
        public void run() {
            synchronized (mRows) {
                ...
                if (DEBUG) {
                    Log.d(TAG, "  launchable activities:");
                }
                for (LauncherActivityInfo lai : lais) {
                    if (DEBUG) {
                        Log.d(TAG, "    " + lai.getComponentName().toString());
                    }
                ...
                if (DEBUG) {
                    Log.d(TAG, "Collected " + mRows.size() + " apps in " + elapsed + "ms");
                }
            }
        }
    };
