...
public final class ProcStatsEntry implements Parcelable {
    ...
    public ProcStatsEntry(ProcessStats.ProcessState proc, String packageName,
        ...
        if (DEBUG) {
            Log.d(TAG, "New proc entry " + proc.mName + ": dur=" + mDuration
    ...
    public void evaluateTargetPackage(PackageManager pm, ProcessStats stats,
        ...
        if (mPackages.size() == 1) {
            ...
            for (int ipkg = 0; ipkg < mPackages.size(); ipkg++) {
                SparseArray localSparseArray = stats.mPackages.get(mPackages.get(ipkg), mUid);
                //ProcessStats.PackageState pkgState = stats.mPackages.get(mPackages.get(ipkg), mUid);
                for (int pkg = 0; pkg < localSparseArray.size(); pkg++) {
                    ProcessStats.PackageState pkgState = (ProcessStats.PackageState)localSparseArray.valueAt(pkg);
                if (DEBUG) {
                    Log.d(TAG, "Eval pkg of " + mName + ", pkg "
                                + pkgState + ":");
                }
                if (pkgState == null) {
            ...
            if (subProcs.size() > 1) {
                ...
                if (subProcs.get(0).mWeight > (subProcs.get(1).mWeight * 3)) {
                    if (DEBUG) {
                        Log.d(TAG, "Eval pkg of " + mName + ": best pkg "
                ...
                for (int i = 0; i < subProcs.size(); i++) {
                    if (subProcs.get(i).mWeight < (maxWeight / 2)) {
                        if (DEBUG) {
                            Log.d(TAG, "Eval pkg of " + mName + ": pkg "
                                    + subProcs.get(i).mPackage + " weight "
                                    + subProcs.get(i).mWeight
                                    + " too small");
                        }
                        continue;
                    }
                    ...
                    if (subProcServices != null) {
                        for (int iss = 0, nss = subProcServices.size(); iss < nss; iss++) {
                            ...
                            if (service.mDuration > thisRunTime) {
                                if (DEBUG) {
                                    Log.d(TAG, "Eval pkg of " + mName + ": pkg "
                                            + subProcs.get(i).mPackage + " service "
                                            + service.mName
                    ...
                    if (thisRunTime > bestRunTime) {
                        if (DEBUG) {
                            Log.d(TAG, "Eval pkg of " + mName + ": pkg "
                                    + subProcs.get(i).mPackage + " new best run time "
                                    + thisRunTime);
                        }
