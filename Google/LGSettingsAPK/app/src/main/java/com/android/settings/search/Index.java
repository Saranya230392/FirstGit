...
import static com.lge.provider.LGSearchIndexablesContract.COLUMN_INDEX_NON_INDEXABLE_KEYS_KEY_VALUE;
import static com.lge.provider.LGSearchIndexablesContract.COLUMN_INDEX_RAW_RANK;
import static com.lge.provider.LGSearchIndexablesContract.COLUMN_INDEX_RAW_TITLE;
import static com.lge.provider.LGSearchIndexablesContract.COLUMN_INDEX_RAW_SUMMARY_ON;
import static com.lge.provider.LGSearchIndexablesContract.COLUMN_INDEX_RAW_SUMMARY_OFF;
import static com.lge.provider.LGSearchIndexablesContract.COLUMN_INDEX_RAW_ENTRIES;
import static com.lge.provider.LGSearchIndexablesContract.COLUMN_INDEX_RAW_KEYWORDS;
import static com.lge.provider.LGSearchIndexablesContract.COLUMN_INDEX_RAW_SCREEN_TITLE;
import static com.lge.provider.LGSearchIndexablesContract.COLUMN_INDEX_RAW_CLASS_NAME;
import static com.lge.provider.LGSearchIndexablesContract.COLUMN_INDEX_RAW_ICON_RESID;
import static com.lge.provider.LGSearchIndexablesContract.COLUMN_INDEX_RAW_INTENT_ACTION;
import static com.lge.provider.LGSearchIndexablesContract.COLUMN_INDEX_RAW_INTENT_TARGET_PACKAGE;
import static com.lge.provider.LGSearchIndexablesContract.COLUMN_INDEX_RAW_INTENT_TARGET_CLASS;
import static com.lge.provider.LGSearchIndexablesContract.COLUMN_INDEX_RAW_KEY;
import static com.lge.provider.LGSearchIndexablesContract.COLUMN_INDEX_RAW_USER_ID;
...
import static com.lge.provider.LGSearchIndexablesContract.COLUMN_INDEX_RAW_CURRENT_ENABLE;
import static com.lge.provider.LGSearchIndexablesContract.COLUMN_INDEX_RAW_PREPERENCE_TYPE;
import static com.lge.provider.LGSearchIndexablesContract.COLUMN_INDEX_RAW_SETTINGS_DB_TABLE;
import static com.lge.provider.LGSearchIndexablesContract.COLUMN_INDEX_RAW_SETTINGS_DB_FIELD;
import static com.lge.provider.LGSearchIndexablesContract.COLUMN_INDEX_RAW_VISIBLE;
import static com.lge.provider.LGSearchIndexablesContract.COLUMN_INDEX_RAW_CHECK_VALUE;
...
public class Index {
    ...
    public static final int COLUMN_INDEX_CURRENT_ENABLE = 15;
    public static final int COLUMN_INDEX_PREPERENCE_TYPE = 16;
    public static final int COLUMN_INDEX_SETTINGS_DB_TABLE = 17;
    public static final int COLUMN_INDEX_SETTINGS_DB_FIELD = 18;
    public static final int COLUMN_INDEX_VISIBLE = 19;
    public static final int COLUMN_INDEX_CHECK_VALUE = 20;

    public static final String ENTRIES_SEPARATOR = "|";

    // If you change the order of columns here, you SHOULD change the
    // COLUMN_INDEX_XXX values
    private static final String[] SELECT_COLUMNS = new String[] {
            ...
            IndexColumns.USER_ID, IndexColumns.CURRENT_ENABLE,
            IndexColumns.PREPERENCE_TYPE, IndexColumns.SETTINGS_DB_TABLE,
            IndexColumns.SETTINGS_DB_FIELD, IndexColumns.VISIBLE,
            IndexColumns.CHECK_VALUE };

    private static final String[] MATCH_COLUMNS_PRIMARY = {
            IndexColumns.DATA_TITLE, IndexColumns.DATA_TITLE_NORMALIZED,
    ...
    // Max number of saved search queries (who will be used for proposing
    // suggestions)
    private static long sMAX_SAVED_SEARCH_QUERY = 64;
    ...
    private static final Pattern REMOVE_DIACRITICALS_PATTERN = Pattern
    ...
    public Cursor search(String query) {
        ...
        final String primarySql = buildSearchSQL(query, MATCH_COLUMNS_PRIMARY,
                true);
        Log.d(LOG_TAG, "Search primary query: " + primarySql);
        cursors[0] = database.rawQuery(primarySql, null);

        // We need to use an EXCEPT operator as negate MATCH queries do not
        // work.
        StringBuilder sql = new StringBuilder(buildSearchSQL(query,
                MATCH_COLUMNS_SECONDARY, false));
        sql.append(" EXCEPT ");
    ...
    public void update() {
        final Intent intent = new Intent(
                LGSearchIndexablesContract.PROVIDER_INTERFACE);
    ...
    private boolean addIndexablesFromRemoteProvider(String packageName,
            String authority) {
        try {
            final int baseRank = Ranking.getBaseRankForAuthority(authority);

            final Context context = mBaseAuthority.equals(authority) ? mContext
                    : mContext.createPackageContext(packageName, 0);

            final Uri uriForResources = buildUriForXmlResources(authority);
            addIndexablesForXmlResourceUri(context, packageName,
                    uriForResources,
                    LGSearchIndexablesContract.INDEXABLES_XML_RES_COLUMNS,
                    baseRank);

            final Uri uriForRawData = buildUriForRawData(authority);
            addIndexablesForRawDataUri(context, packageName, uriForRawData,
                    LGSearchIndexablesContract.INDEXABLES_RAW_COLUMNS, baseRank);
            return true;
    ...
    private void addNonIndexablesKeysFromRemoteProvider(String packageName,
            String authority) {
        final List<String> keys = getNonIndexablesKeysFromRemoteProvider(
                packageName, authority);
        addNonIndexableKeys(packageName, keys);
    }

    private List<String> getNonIndexablesKeysFromRemoteProvider(
            String packageName, String authority) {
        try {
            final Context packageContext = mContext.createPackageContext(
                    packageName, 0);
            ...
            return getNonIndexablesKeys(packageContext, uriForNonIndexableKeys,
                    LGSearchIndexablesContract.NON_INDEXABLES_KEYS_COLUMNS);
        } catch (PackageManager.NameNotFoundException e) {
            Log.w(LOG_TAG, "Could not create context for " + packageName + ": "
                    + Log.getStackTraceString(e));
            return EMPTY_LIST;
            ...
            if (count > 0) {
                while (cursor.moveToNext()) {
                    final String key = cursor
                            .getString(COLUMN_INDEX_NON_INDEXABLE_KEYS_KEY_VALUE);
                    result.add(key);
                }
            }
            return result;
     ...
     * 
     * - have read/write
     * {@link android.Manifest.permission#READ_SEARCH_INDEXABLES} - be from a
     * privileged package
     */
    private boolean isWellKnownProvider(ResolveInfo info) {
        ...
        Log.d(LOG_TAG, "isWellKnownProvider info : " + info);
        Log.d(LOG_TAG, "isWellKnownProvider package : " + packageName);
        Log.d(LOG_TAG, "isWellKnownProvider authority : " + authority);
        ...
        Log.d(LOG_TAG, "isWellKnownProvider readPermission : " + readPermission);
        Log.d(LOG_TAG, "isWellKnownProvider writePermission : " + writePermission);
        ...
        if (!android.Manifest.permission.READ_SEARCH_INDEXABLES
                .equals(readPermission)
                || !android.Manifest.permission.READ_SEARCH_INDEXABLES
                        .equals(writePermission)) {
            return false;
        }
     ...
     * 
     * @param className
     *            the class name (typically a fragment name).
     * @param rebuild
     *            true means that you want to delete the data from the Index
     *            first.
     * @param includeInSearchResults
    ...
    public void updateFromClassNameResource(String className, boolean rebuild,
        ...
        final SearchIndexableResource res = SearchIndexableResources
                .getResourceByName(className);
        if (res == null) {
            Log.e(LOG_TAG,
                    "Cannot find SearchIndexableResources for class name: "
                            + className);
            return;
        }
    ...
    private static Uri buildUriForXmlResources(String authority) {
        return Uri.parse("content://" + authority + "/"
                + LGSearchIndexablesContract.INDEXABLES_XML_RES_PATH);
    }

    private static Uri buildUriForRawData(String authority) {
        return Uri.parse("content://" + authority + "/"
                + LGSearchIndexablesContract.INDEXABLES_RAW_PATH);
    }

    private static Uri buildUriForNonIndexableKeys(String authority) {
        return Uri.parse("content://" + authority + "/"
                + LGSearchIndexablesContract.NON_INDEXABLES_KEYS_PATH);
    }
    ...
    private void addIndexablesForXmlResourceUri(Context packageContext,
            String packageName, Uri uri, String[] projection, int baseRank) {
        ...
        try {
            ...
            if (count > 0) {
                while (cursor.moveToNext()) {
                    final int providerRank = cursor
                            .getInt(COLUMN_INDEX_XML_RES_RANK);
                    final int rank = (providerRank > 0) ? baseRank
                            + providerRank : baseRank;

                    final int xmlResId = cursor
                            .getInt(COLUMN_INDEX_XML_RES_RESID);

                    final String className = cursor
                            .getString(COLUMN_INDEX_XML_RES_CLASS_NAME);
                    final int iconResId = cursor
                            .getInt(COLUMN_INDEX_XML_RES_ICON_RESID);

                    final String action = cursor
                            .getString(COLUMN_INDEX_XML_RES_INTENT_ACTION);
                    final String targetPackage = cursor
                            .getString(COLUMN_INDEX_XML_RES_INTENT_TARGET_PACKAGE);
                    final String targetClass = cursor
                            .getString(COLUMN_INDEX_XML_RES_INTENT_TARGET_CLASS);

                    SearchIndexableResource sir = new SearchIndexableResource(
                            packageContext);
                    sir.rank = rank;
    ...
    private void addIndexablesForRawDataUri(Context packageContext,
            String packageName, Uri uri, String[] projection, int baseRank) {
        ...
        if (cursor == null) {
            ...
            Log.w(LOG_TAG, "Enter addIndexablesForRawDataUri");
            Log.w(LOG_TAG, "Cannot add index data for Uri: " + uri.toString());
            return;
        }
        ...
        try {
            ...
            if (count > 0) {
                while (cursor.moveToNext()) {
                    final int providerRank = cursor
                            .getInt(COLUMN_INDEX_RAW_RANK);
                    final int rank = (providerRank > 0) ? baseRank
                            + providerRank : baseRank;

                    final String title = cursor
                            .getString(COLUMN_INDEX_RAW_TITLE);
                    final String summaryOn = cursor
                            .getString(COLUMN_INDEX_RAW_SUMMARY_ON);
                    final String summaryOff = cursor
                            .getString(COLUMN_INDEX_RAW_SUMMARY_OFF);
                    final String entries = cursor
                            .getString(COLUMN_INDEX_RAW_ENTRIES);
                    final String keywords = cursor
                            .getString(COLUMN_INDEX_RAW_KEYWORDS);

                    final String screenTitle = cursor
                            .getString(COLUMN_INDEX_RAW_SCREEN_TITLE);

                    final String className = cursor
                            .getString(COLUMN_INDEX_RAW_CLASS_NAME);
                    final int iconResId = cursor
                            .getInt(COLUMN_INDEX_RAW_ICON_RESID);

                    final String action = cursor
                            .getString(COLUMN_INDEX_RAW_INTENT_ACTION);
                    final String targetPackage = cursor
                            .getString(COLUMN_INDEX_RAW_INTENT_TARGET_PACKAGE);
                    final String targetClass = cursor
                            .getString(COLUMN_INDEX_RAW_INTENT_TARGET_CLASS);

                    final String key = cursor.getString(COLUMN_INDEX_RAW_KEY);
                    final int userId = cursor.getInt(COLUMN_INDEX_RAW_USER_ID);
                    final int currentEnable = cursor
                            .getInt(COLUMN_INDEX_RAW_CURRENT_ENABLE);
                    final String preferenceType = cursor
                            .getString(COLUMN_INDEX_RAW_PREPERENCE_TYPE);
                    final String settingsDBTableName = cursor
                            .getString(COLUMN_INDEX_RAW_SETTINGS_DB_TABLE);
                    final String settingsDBField = cursor
                            .getString(COLUMN_INDEX_RAW_SETTINGS_DB_FIELD);
                    final int visible = cursor.getInt(COLUMN_INDEX_RAW_VISIBLE);
                    final int checkValue = cursor
                            .getInt(COLUMN_INDEX_RAW_CHECK_VALUE);
                    SearchIndexableRaw data = new SearchIndexableRaw(
                            packageContext);
                    data.rank = rank;
                    data.title = title;
    ...
    private String buildSearchSQL(String query, String[] colums,
            boolean withOrderBy) {
        StringBuilder sb = new StringBuilder();
    ...
    private String buildSearchWhereStringForColumns(String query,
            String[] columnNames) {
        ...
        sb.append(" = ");
        DatabaseUtils
                .appendEscapedSQLString(sb, Locale.getDefault().toString());
        sb.append(" AND ");
    ...
    private String buildSearchMatchStringForColumns(String query,
            String[] columnNames) {
    ...
    private void indexOneSearchIndexableData(SQLiteDatabase database,
            String localeStr, SearchIndexableData data,
            Map<String, List<String>> nonIndexableKeys) {
        if (data instanceof SearchIndexableResource) {
            indexOneResource(database, localeStr,
                    (SearchIndexableResource)data, nonIndexableKeys);
    ...
    private void indexOneRaw(SQLiteDatabase database, String localeStr,
        ...
        updateOneRowWithFilteredData(database, localeStr, raw.title,
                raw.summaryOn, raw.summaryOff, raw.entries, raw.className,
                raw.screenTitle, raw.iconResId, raw.rank, raw.keywords,
                raw.intentAction, raw.intentTargetPackage,
                raw.intentTargetClass, raw.enabled, raw.key, raw.userId,
                raw.currentEnable, raw.preferenceType, raw.settingsDBTableName,
                raw.settingsDBField, raw.visible, raw.checkValue);
    }
    ...
    private void indexOneResource(SQLiteDatabase database, String localeStr,
            ...
            Map<String, List<String>> nonIndexableKeysFromResource) {
        ...
        if (sir.xmlResId > SearchIndexableResources.sNO_DATA_RES_ID) {
            List<String> resNonIndxableKeys = nonIndexableKeysFromResource
                    .get(sir.packageName);
            if (resNonIndxableKeys != null && resNonIndxableKeys.size() > 0) {
                nonIndexableKeys.addAll(resNonIndxableKeys);
            }

            indexFromResource(sir.context, database, localeStr, sir.xmlResId,
                    sir.className, sir.iconResId, sir.rank, sir.intentAction,
                    sir.intentTargetPackage, sir.intentTargetClass,
                    nonIndexableKeys, -1, null, null, null, 0, 0);
        } else {
            ...
            if (provider != null) {
                List<String> providerNonIndexableKeys = provider
                        .getNonIndexableKeys(sir.context);
                if (providerNonIndexableKeys != null
                        && providerNonIndexableKeys.size() > 0) {
                    nonIndexableKeys.addAll(providerNonIndexableKeys);
                }

                indexFromProvider(mContext, database, localeStr, provider,
                        sir.className, sir.iconResId, sir.rank, sir.enabled,
                        nonIndexableKeys);
            }
        }
    }

    private Indexable.SearchIndexProvider getSearchIndexProvider(
            final Class<?> clazz) {
        try {
            final Field f = clazz
                    .getField(FIELD_NAME_SEARCH_INDEX_DATA_PROVIDER);
            return (Indexable.SearchIndexProvider)f.get(null);
        } catch (NoSuchFieldException e) {
            Log.d(LOG_TAG, "Cannot find field '"
                    + FIELD_NAME_SEARCH_INDEX_DATA_PROVIDER + "'");
        } catch (SecurityException se) {
            Log.d(LOG_TAG, "Security exception for field '"
                    + FIELD_NAME_SEARCH_INDEX_DATA_PROVIDER + "'");
        } catch (IllegalAccessException e) {
            Log.d(LOG_TAG, "Illegal access to field '"
                    + FIELD_NAME_SEARCH_INDEX_DATA_PROVIDER + "'");
        } catch (IllegalArgumentException e) {
            Log.d(LOG_TAG, "Illegal argument when accessing field '"
                    + FIELD_NAME_SEARCH_INDEX_DATA_PROVIDER + "'");
        }
        return null;
    }

    private void indexFromResource(Context context, SQLiteDatabase database,
            String localeStr, int xmlResId, String fragmentName, int iconResId,
            int rank, String intentAction, String intentTargetPackage,
            String intentTargetClass, List<String> nonIndexableKeys,
            int currentEnable, String preferenceType,
            String settingsDBTableName, String settingsDBField, int visible,
            int checkValue) {
        ...
        try {
            ...
            // Insert rows for the main PreferenceScreen node. Rewrite the data
            // for removing
            // hyphens.
            if (!nonIndexableKeys.contains(key)) {
                ...
                updateOneRowWithFilteredData(database, localeStr, title,
                        summary, null, null, fragmentName, screenTitle,
                        iconResId, rank, keywords, intentAction,
                        intentTargetPackage, intentTargetClass, true, key,
                        -1 /* default user id */, currentEnable,
                        preferenceType, settingsDBTableName, settingsDBField,
                        visible, checkValue);
            }
            ...
            while ((type = parser.next()) != XmlPullParser.END_DOCUMENT
                ...
                if (!nodeName.equals(NODE_NAME_CHECK_BOX_PREFERENCE)) {
                    ...
                    updateOneRowWithFilteredData(database, localeStr, title,
                            summary, null, entries, fragmentName, screenTitle,
                            iconResId, rank, keywords, intentAction,
                            intentTargetPackage, intentTargetClass, true, key,
                            -1 /* default user id */, currentEnable,
                            preferenceType, settingsDBTableName,
                    ...
                    if (TextUtils.isEmpty(summaryOn)
                            && TextUtils.isEmpty(summaryOff)) {
                        summaryOn = getDataSummary(context, attrs);
                    }

                    updateOneRowWithFilteredData(database, localeStr, title,
                            summaryOn, summaryOff, null, fragmentName,
                            screenTitle, iconResId, rank, keywords,
                            intentAction, intentTargetPackage,
                            intentTargetClass, true, key,
                            -1 /* default user id */, currentEnable,
                            preferenceType, settingsDBTableName,
                            settingsDBField, visible, checkValue);
                }
    ...
    private void indexFromProvider(Context context, SQLiteDatabase database,
            String localeStr, Indexable.SearchIndexProvider provider,
            String className, int iconResId, int rank, boolean enabled,
        ...
        final List<SearchIndexableRaw> rawList = provider.getRawDataToIndex(
                context, enabled);
        ...
        if (rawList != null) {
            ...
            for (int i = 0; i < rawSize; i++) {
                ...
                updateOneRowWithFilteredData(database, localeStr, raw.title,
                        raw.summaryOn, raw.summaryOff, raw.entries, className,
                        raw.screenTitle, iconResId, rank, raw.keywords,
                        raw.intentAction, raw.intentTargetPackage,
                        raw.intentTargetClass, raw.enabled, raw.key,
                        raw.userId, raw.currentEnable, raw.preferenceType,
                        raw.settingsDBTableName, raw.settingsDBField,
                        raw.visible, raw.checkValue);
            }
        }

        final List<SearchIndexableResource> resList = provider
                .getXmlResourcesToIndex(context, enabled);
        if (resList != null) {
            ...
            for (int i = 0; i < resSize; i++) {
                ...
                String itemClassName = (TextUtils.isEmpty(item.className)) ? className
                        : item.className;

                indexFromResource(context, database, localeStr, item.xmlResId,
                        itemClassName, itemIconResId, itemRank,
                        item.intentAction, item.intentTargetPackage,
                        item.intentTargetClass, nonIndexableKeys, -1, null,
                        null, null, 0, 0);
            }
        }
    }

    private void updateOneRowWithFilteredData(SQLiteDatabase database,
            String locale, String title, String summaryOn, String summaryOff,
            String entries, String className, String screenTitle,
            int iconResId, int rank, String keywords, String intentAction,
            String intentTargetPackage, String intentTargetClass,
            boolean enabled, String key, int userId, int currentEnable,
        ...
        updateOneRow(database, locale, updatedTitle, normalizedTitle,
                updatedSummaryOn, normalizedSummaryOn, updatedSummaryOff,
                normalizedSummaryOff, entries, className, screenTitle,
                iconResId, rank, keywords, intentAction, intentTargetPackage,
                intentTargetClass, enabled, key, userId, currentEnable,
                preferenceType, settingsDBTableName, settingsDBField, visible,
                checkValue);
    }
    ...
    private static String normalizeString(String input) {
        final String nohyphen = (input != null) ? input.replaceAll(HYPHEN,
                EMPTY) : EMPTY;
        final String normalized = Normalizer.normalize(nohyphen,
                Normalizer.Form.NFD);

        return REMOVE_DIACRITICALS_PATTERN.matcher(normalized).replaceAll("")
                .toLowerCase();
    }
    ...
    private void updateOneRow(SQLiteDatabase database, String locale,
            ...
            String updatedSummaryOff, String normalizedSummaryOff,
            String entries, String className, String screenTitle,
            int iconResId, int rank, String keywords, String intentAction,
            String intentTargetPackage, String intentTargetClass,
            boolean enabled, String key, int userId, int currentEnable,
        ...
        // The DocID should contains more than the title string itself (you may
        // have two settings
        // with the same title). So we need to use a combination of the title
        // and the screenTitle.
        ...
        values.put(IndexColumns.DATA_SUMMARY_OFF_NORMALIZED,
        ...
        values.put(IndexColumns.CURRENT_ENABLE, currentEnable);
        values.put(IndexColumns.PREPERENCE_TYPE, preferenceType);
        values.put(IndexColumns.SETTINGS_DB_TABLE, settingsDBTableName);
        values.put(IndexColumns.SETTINGS_DB_FIELD, settingsDBField);
        values.put(IndexColumns.VISIBLE, visible);
        values.put(IndexColumns.CHECK_VALUE, checkValue);
    ...
    private String getData(Context context, AttributeSet set, int[] attrs,
            int resId) {
    ...
    private String getDataEntries(Context context, AttributeSet set,
            int[] attrs, int resId) {
        final TypedArray sa = context.obtainStyledAttributes(set, attrs);
        final TypedValue tv = sa.peekValue(resId);
    ...
    private int getResId(Context context, AttributeSet set, int[] attrs,
            int resId) {
        final TypedArray sa = context.obtainStyledAttributes(set, attrs);
        final TypedValue tv = sa.peekValue(resId);
    ...
    private class UpdateIndexTask extends AsyncTask<UpdateData, Integer, Void> {
        ...
        private boolean processDataToUpdate(SQLiteDatabase database,
                String localeStr, List<SearchIndexableData> dataToUpdate,
                Map<String, List<String>> nonIndexableKeys, boolean forceUpdate) {
            ...
            for (int n = 0; n < count; n++) {
                ...
                try {
                    ...
                    Log.e(LOG_TAG, "Cannot index: " + data.className
        ...
        private boolean processDataToDelete(SQLiteDatabase database,
                String localeStr, List<SearchIndexableData> dataToDelete) {
        ...
        private int delete(SQLiteDatabase database, String columName,
                String value) {
            final String whereClause = columName + "=?";
            final String[] whereArgs = new String[] { value };

            return database.delete(Tables.TABLE_PREFS_INDEX, whereClause,
                    whereArgs);
        }

        private boolean isLocaleAlreadyIndexed(SQLiteDatabase database,
                String locale) {
            Cursor cursor = null;
            boolean result = false;
    ...
    private class SaveSearchQueryTask extends AsyncTask<String, Void, Long> {
        ...
        protected Long doInBackground(String... params) {
            ...
            try {
                ...
                lastInsertedRowId = database.insertOrThrow(
                        Tables.TABLE_SAVED_QUERIES, null, values);

                // Last, remove "old" saved queries
                final long delta = lastInsertedRowId - sMAX_SAVED_SEARCH_QUERY;
                if (delta > 0) {
                    int count = database
                            .delete(Tables.TABLE_SAVED_QUERIES, "rowId <= ?",
                                    new String[] { Long.toString(delta) });
                    Log.d(LOG_TAG, "Deleted '" + count
                            + "' saved Search query(ies)");
                }
            } catch (Exception e) {
