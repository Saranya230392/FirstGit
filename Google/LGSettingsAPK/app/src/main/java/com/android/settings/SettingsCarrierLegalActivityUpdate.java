...
public class SettingsCarrierLegalActivityUpdate extends Activity implements
		DialogInterface.OnCancelListener, DialogInterface.OnClickListener {

	private static final String TAG = "SettingsCarrierLegalActivityUpdate";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Log.i(TAG,"bcheck_HFA_Status = "+ bcheck_HFA_Status +", bcheck_Payload = " + bcheck_Payload);
...
//                  myDatabase.insert(DB_TABLE, null, insert_db_item(getResources().getStringArray(R.array.sp_carrier_legal_virgin_NORMAL)));
            ...
            try {
                ...
                if (myDatabase != null && myDatabase.isOpen()) {
                    if (bPayload_Data) {// Boost Mobile Carrier Legal database create
                        if ("BM".equals(Config.getOperator())) {
                            myDatabase.insert(DB_TABLE, null, insert_db_item(getResources().getStringArray(R.array.sp_carrier_legal_bm_NORMAL)));
                        } else {
                            myDatabase.insert(DB_TABLE, null, insert_db_item(getResources().getStringArray(R.array.sp_carrier_legal_spr_NORMAL)));
                        }
                    } else {  // generic Carrier Legal database create
                        myDatabase.insert(DB_TABLE, null, insert_db_item(getResources().getStringArray(R.array.sp_carrier_legal_NORMAL)));
                    }
                } else {
                    Log.e(TAG,"myDatabase == null or createDataBase isOpen status :"+ myDatabase.isOpen());
                }
            } finally {
               if (myDatabase != null) {
                    myDatabase.close();
	...
	private boolean set_value_from_db(String db_name) {
		...
		try {
			Log.i(TAG, "openOrCreateDatabase" + db_name);
			...
			if(db_name.equals(CARRIER_DB))
			{
			    ...
			    myDatabase = SQLiteDatabase.openDatabase(db_name, null, SQLiteDatabase.OPEN_READONLY | SQLiteDatabase.NO_LOCALIZED_COLLATORS);
			}
			else
			{
	...
	private void PrivacyAlertDialog() {
		...
		mPrivacyAlert.setMessage(mAlert);
		mPrivacyAlert.show();
	}
	...
	private void PrivacyAlert_Detail() {
		...
		mPrivacyDetail.setOnDismissListener(new OnDismissListener() {
			...
			public void onDismiss(DialogInterface arg0) {
				...
				SettingsCarrierLegalActivityUpdate.this.finish();
			}
		});
	}
