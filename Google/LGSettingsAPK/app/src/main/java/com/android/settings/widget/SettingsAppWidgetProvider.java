...
public class SettingsAppWidgetProvider extends AppWidgetProvider {
    ...
    private static final String DEVICE_ADMIN_EXTRA_SERVICE = "device_admin_extra_service";
    private static final String DEVICE_ADMIN_SERVICE_BLUETOOTH = "device_admin_service_bluetooth";
    ...
    private static final int[] IND_DRAWABLE_OFF = {
            R.drawable.img_appwidget_settings_ind_off_l_holo,
            R.drawable.img_appwidget_settings_ind_off_c_holo,
            R.drawable.img_appwidget_settings_ind_off_r_holo
    };

    private static final int[] IND_DRAWABLE_MID = {
            R.drawable.img_appwidget_settings_ind_mid_l_holo,
            R.drawable.img_appwidget_settings_ind_mid_c_holo,
            R.drawable.img_appwidget_settings_ind_mid_r_holo
    };

    private static final int[] IND_DRAWABLE_ON = {
            R.drawable.img_appwidget_settings_ind_on_l_holo,
            R.drawable.img_appwidget_settings_ind_on_c_holo,
            R.drawable.img_appwidget_settings_ind_on_r_holo
    };
    ...
    private static final StateTracker LOCATION_STATE = new LocationStateTracker();
        ...
        public void onActualStateChange(Context context, Intent intent) {
            ...
            if (com.android.settings.lgesetting.Config.Config.THREELM_MDM) {
                if (!(BluetoothAdapter.ACTION_STATE_CHANGED.equals(intent.getAction()))
                        && !(BluetoothDeviceAdminStateReceiver.INTENT_ADMIN_ACTION_STATE_CHANGED
                                .equals(
                                intent.getAction()))) {
                    return;
                }
    ...
    private static void updateButtons(RemoteViews views, Context context) {
        ...
        LOCATION_STATE.setImageViewResources(context, views);
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (WifiManager.WIFI_STATE_CHANGED_ACTION.equals(action)) {
            ...
            // 3LM_MDM_START
                ...
                && BluetoothDeviceAdminStateReceiver.INTENT_ADMIN_ACTION_STATE_CHANGED
                        .equals(action)) {
            String service = intent.getStringExtra(DEVICE_ADMIN_EXTRA_SERVICE);
            if (service.equals(DEVICE_ADMIN_SERVICE_BLUETOOTH)) {
                sBluetoothState.onActualStateChange(context, intent);
            }
            // 3LM_MDM_START
        } else if (LocationManager.MODE_CHANGED_ACTION.equals(action)) {
            LOCATION_STATE.onActualStateChange(context, intent);
            ...
            if (buttonId == BUTTON_WIFI) {
                ...
                LOCATION_STATE.toggleState(context);
    ...
    // 3LM_MDM_START KK
    // SettingsAppWidgetProvider is stopped receiving broadcasts from external apps.
    // Added a new receiver to receive device admin notifications
    public static class BluetoothDeviceAdminStateReceiver extends BroadcastReceiver {
        public static final String INTENT_ADMIN_ACTION_STATE_CHANGED = "threelm.admin.action.STATE_CHANGED";

        @Override
        public void onReceive(Context context, Intent intent) {
            Intent localIntent = new Intent(INTENT_ADMIN_ACTION_STATE_CHANGED);
            localIntent.putExtras(intent);
            context.sendBroadcast(localIntent, android.Manifest.permission.BLUETOOTH);
        }
    }
    // 3LM_MDM_END
