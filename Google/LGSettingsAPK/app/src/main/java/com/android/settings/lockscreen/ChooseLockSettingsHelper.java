...
// LGMDM [a1-mdm-dev@lge.com][ID-MDM-10]
import com.lge.constants.LockPatternUtilsConstants;
// LGMDM_END
...
public final class ChooseLockSettingsHelper {
    ...
    private static final String LOCK_SETTING_PACKAGE_NAME = "com.lge.lockscreensettings";

    private LockPatternUtilsEx mLockPatternUtils;
    private Activity mActivity;
    private Fragment mFragment;
    // LGMDM [a1-mdm-dev@lge.com][ID-MDM-10]
    private boolean mPasswordExpired = false;
    // LGMDM_END

    public ChooseLockSettingsHelper(Activity activity) {
        mActivity = activity;
        mLockPatternUtils = new LockPatternUtilsEx(activity);
    }
    ...
    public boolean launchConfirmationActivity(int request, CharSequence message, CharSequence details) {
        ...
        switch (mLockPatternUtils.getKeyguardStoredPasswordQuality()) {
            // Add password quality of knock code
            case DevicePolicyManagerConstants.PASSWORD_QUALITY_KNOCK_ON:
                launched = confirmKnockOn(request);
                break;
    ...
    private boolean confirmPattern(int request, CharSequence message, CharSequence details) {
        ...
        intent.setClassName(LOCK_SETTING_PACKAGE_NAME, "com.lge.lockscreensettings.lockscreen.ConfirmLockPattern");
        if (mFragment != null) {
    ...
    private boolean confirmPassword(int request) {
        ...
        final Intent intent = new Intent();
        intent.setClassName(LOCK_SETTING_PACKAGE_NAME, "com.lge.lockscreensettings.lockscreen.ConfirmLockPassword");
        // LGMDM [a1-mdm-dev@lge.com][ID-MDM-10]
        if (com.lge.config.ConfigBuildFlags.CAPP_MDM && mPasswordExpired) {
            intent.putExtra(LockPatternUtilsConstants.PASSWORD_EXPIRE_MODE, true);
        }
        // LGMDM_END
        if (mFragment != null) {
            mFragment.startActivityForResult(intent, request);
    ...
    /**
     * Launch screen to confirm the existing lock knoclOn.
     * @see #onActivityResult(int, int, android.content.Intent)
     * @return true if we launched an activity to confirm password
     */
    private boolean confirmKnockOn(int request) {
        if (!mLockPatternUtils.isLockKnockOnEnabled()) {
            return false;
        }
        final Intent intent = new Intent();
        intent.setClassName(LOCK_SETTING_PACKAGE_NAME, "com.lge.lockscreensettings.lockscreen.ConfirmLockKnockOn");
        // LGMDM [a1-mdm-dev@lge.com][ID-MDM-10]
        if (com.lge.config.ConfigBuildFlags.CAPP_MDM && mPasswordExpired) {
            intent.putExtra(LockPatternUtilsConstants.PASSWORD_EXPIRE_MODE, true);
        }
