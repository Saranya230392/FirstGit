...
public class TtsEnginePreference extends Preference {
    ...
    // [2012.02.18][seungeene[NA] Apply disabled alpha level for mSettingsIcon when it is disable.
    ...
    public View getView(View convertView, ViewGroup parent) {
        ...
        rb = (RadioButton)view.findViewById(R.id.tts_engine_radiobutton);
        rb.setOnCheckedChangeListener(mRadioChangeListener);

        // getKey() : current Preference
        // mSharedState.getCurrentKey() : selected Preference
        boolean isChecked = getKey().equals(mSharedState.getCurrentKey());
        Log.d(TAG, "getView : isChecked: " + isChecked + " :key:" + getKey());
        if (isChecked) {
        ...
        textLayout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Log.d(TAG, "textLayout.onClick()");
                onRadioButtonClicked(rb, !rb.isChecked());
                try {
                    if ((mSharedState != null) && (getKey().equals(mSharedState.getCurrentKey()))) {
                        updateSettingsIconViews(true);
                        Log.d(TAG, "textLayout no change return");
                        return;
                    }
                } catch (Exception e) {
                    Log.e("[TtsEnginePreference]", "Exception : " + e);
                }
                onRadioButtonClicked(rb, rb.isChecked());
            }
        });

        View ttsEnginePref = view.findViewById(R.id.tts_engine_pref);
        ttsEnginePref.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Log.d(TAG, "mTtsEnginePref.onClick(): key:" + getKey());
                    ...
                    mSharedState.setCurrentChecked(mRadioButton);
                    mSharedState.setCurrentKey(getKey());
                    callChangeListener(mSharedState.getCurrentKey());
                }
            });
        // [E][2012.02.18][seungeene[TD_135400] change tts listener object (tts_engine_pref are only touched)
    ...
    private void updateSettingsIconViews(boolean checked) {
        if (mSettingsIcon != null) {
            mSettingsIcon.setEnabled(checked);
            mSettingsIcon.setClickable(checked);
            mSettingsIcon.setFocusable(checked);

            if (!checked) {
                mSettingsIcon.setAlpha(DISABLED_ALPHA);
            } else {
                mSettingsIcon.setAlpha(ENABLED_ALPHA);
            }
        }
    }
    ...
    public boolean onKey(View v, int keyCode, KeyEvent event) {
        ...
        if (keyCode == KeyEvent.KEYCODE_ENTER && event.getAction() == KeyEvent.ACTION_DOWN) {
			boolean isChecked = getKey().equals(mSharedState.getCurrentKey());
			Log.i(TAG, "TTS onKey : isChecked : " + isChecked + " :key: " + getKey() );
			if (!isChecked) {
				Log.d(TAG, "TTS onKey getCurrentChecked() : " + mSharedState.getCurrentChecked());
				if (mSharedState.getCurrentChecked() != null) {
					mSharedState.getCurrentChecked().setChecked(false);
				}
				mSharedState.setCurrentKey(getKey());
