...
public class ChartGridView extends View {
    ...
    private Layout makeLabel(CharSequence text) {
        ...
        final Context context = getContext();
        final int mFixedLabelSize = context.getResources().getDimensionPixelSize(R.dimen.data_usage_chart_date_fixed_font_size);
        paint.density = res.getDisplayMetrics().density;
        paint.setCompatibilityScaling(res.getCompatibilityInfo().applicationScale);
        paint.setColor(mLabelColor);
        Log.d("ChartGridView", "ChartGridView : mLabelSize = " + mLabelSize);
        Log.d("ChartGridView", "ChartGridView : mFixedLabelSize = " + mFixedLabelSize);
        paint.setTextSize(mFixedLabelSize);
