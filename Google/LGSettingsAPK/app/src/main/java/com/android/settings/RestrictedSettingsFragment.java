...
/**
 * Base class for settings screens that should be pin protected when in
 * restricted mode. The constructor for this class will take the restriction key
 * that this screen should be locked by. If {@link
 * RestrictionsManager.hasRestrictionsProvider()} and {@link
 * UserManager.hasUserRestriction()}, then the user will have to enter the
 * restrictions pin before seeing the Settings screen.
 * 
 * If this settings screen should be pin protected whenever {@link
 * RestrictionsManager.hasRestrictionsProvider()} returns true, pass in
 * {@link RESTRICT_IF_OVERRIDABLE} to the constructor instead of a restrictions
 * key.
 */
public class RestrictedSettingsFragment extends SettingsPreferenceFragment {
    ...
    // No RestrictedSettingsFragment screens should use this number in
     ...
     *            The restriction key to check before pin protecting this
     *            settings page. Pass in {@link RESTRICT_IF_OVERRIDABLE} if it
     *            should be protected whenever a restrictions provider is set.
     *            Pass in null if it should never be protected.
     */
    ...
    public void onCreate(Bundle icicle) {
        ...
        if (icicle != null) {
            mChallengeSucceeded = icicle.getBoolean(KEY_CHALLENGE_SUCCEEDED,
                    false);
            mChallengeRequested = icicle.getBoolean(KEY_CHALLENGE_REQUESTED,
                    false);
        }
    ...
    private void ensurePin() {
        if (!mChallengeSucceeded && !mChallengeRequested
            ...
            if (intent != null) {
                ...
                request.putString(
                        RestrictionsManager.REQUEST_KEY_MESSAGE,
                        getResources().getString(
                                R.string.restr_pin_enter_admin_pin));
                intent.putExtra(RestrictionsManager.EXTRA_REQUEST_BUNDLE,
                        request);
    ...
    /**
     * Returns true if this activity is restricted, but no restrictions provider
     * has been set. Used to determine if the settings UI should disable UI.
     */
    protected boolean isRestrictedAndNotProviderProtected() {
        if (mRestrictionKey == null
                || RESTRICT_IF_OVERRIDABLE.equals(mRestrictionKey)) {
            return false;
        }
    ...
    protected boolean hasChallengeSucceeded() {
        return (mChallengeRequested && mChallengeSucceeded)
                || !mChallengeRequested;
    }
    ...
    /**
     * Returns whether restricted or actionable UI elements should be removed or
     * disabled.
     */
    protected boolean isUiRestricted() {
        return isRestrictedAndNotProviderProtected()
                || !hasChallengeSucceeded();
    }
}
