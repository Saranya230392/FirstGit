...
public class ApplicationsState {
    ...
    public static class AppEntry extends SizeInfo {
        ...
        void ensuredate(Context context) {
            if (this.label == null || !this.mounted) {
                if (!this.apkFile.exists()) {
                    ...
                    try {
                        long temp_installed = pm.getPackageInfo(info.packageName, 0).firstInstallTime;
                        getData(temp_installed, context);
        ...
        void getData(long datetime, Context context) {
    ...
    public static final Comparator<AppEntry> ALPHA_COMPARATOR = new Comparator<AppEntry>() {
        ...
        public int compare(AppEntry object1, AppEntry object2) {
            /* yonguk.kim 20120323 Not sort by enable status requested by UI
    ...
    public static final Comparator<AppEntry> DATE_COMPARATOR = new Comparator<AppEntry>() {
        private final Collator sCollator = Collator.getInstance();

        @Override
        public int compare(AppEntry object1, AppEntry object2) {
            return sCollator.compare(object2.installed, object1.installed);
        }
    };
    ...
    private ApplicationsState(Application app) {
        ...
        if (UserHandle.myUserId() == 0) {
            mRetrieveFlags = PackageManager.GET_UNINSTALLED_PACKAGES |
                    ...
                    PackageManager.GET_DISABLED_UNTIL_USED_COMPONENTS; // rebestm - kk migration
        } else {
    ...
    public class Session {
        ...
        public void resume(boolean rebuild) {
            if (DEBUG_LOCKING) {
            ...
            synchronized (mEntriesMap) {
                if (!mResumed) {
                    ...
                    doResumeIfNeededLocked(rebuild);
                }
            }
            if (DEBUG_LOCKING) {
    ...
    void doResumeIfNeededLocked(boolean rebuild) {
        if (mResumed && !mInterestingConfigChanges.applyNewConfig(mContext.getResources())) {
            return;
        }
        mResumed = true;
        ...
        if (mInterestingConfigChanges.applyNewConfig(mContext.getResources()) || rebuild) {
            Log.i(TAG, "NEW Apps list up!!! rebuild = " + rebuild);
        ...
        mHaveDisabledApps = false; // rebestm - kk migration
    ...
    class BackgroundHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            case MSG_LOAD_ICONS: {
                int numDone = 0;
                int i = 0; // yonguk.kim 20120816 ICS Stuff Porting to JB - Handle OutOfMemory Exception
                synchronized (mEntriesMap) {
                    if (DEBUG_LOCKING) {
                        Log.v(TAG, "MSG_LOAD_ICONS acquired lock");
                    }
                    for (i = 0; i < mAppEntries.size() && numDone < 2; i++) {
                ...
                if (numDone >= 2 && i < 10) { // yonguk.kim 20120816 ICS Stuff Porting to JB - Handle OutOfMemory Exception
                    sendEmptyMessage(MSG_LOAD_ICONS);
                } else {
                    sendEmptyMessage(MSG_LOAD_SIZES);
                }
            ...
            case MSG_LOAD_SIZES: {
                synchronized (mEntriesMap) {
                    ...
                    if (mCurComputingSizePkg != null || mAppEntries == null) { // yonguk.kim 20120816 ICS Stuff Porting to JB - Null Check
                        if (DEBUG_LOCKING) {
                    ...
                    for (int i = 0; i < mAppEntries.size(); i++) {
                        AppEntry entry = mAppEntries.get(i);
                        // yonguk.kim 20120816 ICS Stuff Porting to JB - Null Check
                        if (entry == null) {
                            break;
                        }
