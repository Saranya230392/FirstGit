...
public class ChartSweepView extends View {
    ...
    public ChartSweepView(Context context, AttributeSet attrs, int defStyle) {
        ...
        setBackgroundResource(com.lge.internal.R.drawable.list_selector_background);
    ...
    private boolean dispatchVerticalType() {
        boolean ret = false;
        // LGMDM [a1-mdm-dev@lge.com][ID-MDM-344][ID-MDM-346]ID-MDM-345][ID-MDM-347]
        if (mListener != null) {
            ret = mListener.verticalType(this);
        }
        // LGMDM_END
        return ret;
    }
    ...
    private void invalidateLabel() {
        ...
        if (mAxis != null) {
            String instanceName = mAxis.toString();
            if (!instanceName.contains("TimeAxis")) {
                if (mLabelValue > ACTUAL_LIMIT) {
                    mLabelValue = ACTUAL_LIMIT;
                }
            } // kerry
        }
    }
    ...
    public void invalidateLabelOffset() {
        ...
        if (Float.compare(labelOffset, mLabelOffset) != 0) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        case MotionEvent.ACTION_MOVE: {
            ...
            if (null == mTracking || null == mMargins || null == mAxis) {
                return true;
            }
            if (mFollowAxis == VERTICAL) {
                //add to protect the null exception.
                if (null == mTracking) {
                    return true;
                }
                ...
                if (value >= ACTUAL_LIMIT) {
                    value = ACTUAL_LIMIT;
                } // kerry - to avoid endless increase
    ...
    private Rect getParentContentRect() {
        final View parent = (View)getParent();
        return new Rect(parent.getPaddingStart(), parent.getPaddingTop(),
                parent.getWidth() - parent.getPaddingEnd(),
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mFollowAxis == VERTICAL) {
            ...
            mSweep.setBounds(mSweepOffset.x, labelSize,
                    mSweepOffset.x + mSweep.getIntrinsicWidth(),
    ...
    protected void onConfigurationChanged(Configuration newConfig) {
        ...
        MotionEvent event = MotionEvent.obtain(downTime, eventTime, MotionEvent.ACTION_UP, 100,
                100, 0);
        onTouchEvent(event);
    }
}
