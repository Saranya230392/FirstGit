...
import android.graphics.drawable.Drawable.ConstantState; // rebestm - kk migration
...
public class RunningState {
        ...
        boolean updateService(Context context, ActivityManager.RunningServiceInfo service) {
            ...
            if (si == null) {
                ...
                try {
                    ...
                    Log.e("RunningService", " RemoteException :  " + e.getMessage());
                    return false;
                }
                ...
                try {
                    ...
                    Log.e("RunningService", " NullPointerEception :  " + e.getMessage());
                    return false;
                }
    ...
    private boolean update(Context context, ActivityManager am) {
        final PackageManager pm = context.getPackageManager();
        //[S][Jaeyoon.hyun][130712] TD 56077, Remove deleted user's service.
        final List<UserInfo> users = mUm.getUsers();
        //[E][Jaeyoon.hyun][130712] TD 56077, Remove deleted user's service.
        mSequence++;
        ...
        for (int i = 0; i < NS; i++) {
            ...
            for (int j = 0; j < users.size(); j++) {
                Log.i(TAG, "users : " + users.get(j).id);
                if (users.get(j).id == UserHandle.getUserId(si.uid)) {
                    deleted = false;
                }
            }
            Log.i(TAG, "UserID : " + UserHandle.getUserId(si.uid));
            if (deleted) {
                Log.i(TAG, "remove deleted user's service");
                services.remove(i);
                i--;
                NS--;
                continue;
