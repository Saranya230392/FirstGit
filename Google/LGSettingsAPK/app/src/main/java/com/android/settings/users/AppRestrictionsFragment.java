...
public class AppRestrictionsFragment extends SettingsPreferenceFragment implements
    ...
    static class AppRestrictionsPreference extends SwitchPreference {
        ...
        protected void onBindView(View view) {
            ...
            if (widget.getChildCount() > 0) {
            	// there is another view "divider" in SwitchPreference at white theme
            	ViewGroup widgetLayout = (ViewGroup)widget.getChildAt(0);
                final Switch toggle = (Switch)widgetLayout.getChildAt(1);
                ...
                toggle.setOnCheckedChangeListener(new OnCheckedChangeListener() {
                    ...
                    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                        toggle.playSoundEffect(SoundEffectConstants.CLICK);
    ...
    protected void init(Bundle icicle) {
        if (icicle != null) {
            ...
            Log.d("YSY", "icicle != null, mUser : " + mUser);
        } else {
            Bundle args = getArguments();
            Log.d("YSY", "icicle == null");
            if (args != null) {
                ...
                Log.d("YSY", "icicle == null, mNewUser : " + mNewUser);
            }
        }
        ...
        try {
            ...
            Log.w("YSY", "NameNotFoundException");
        }
    ...
    Drawable getCircularUserIcon() {
        ...
        CircleFramedDrawable circularIcon =
                CircleFramedDrawable.getInstance(mContext, userIcon);
        return circularIcon;
    }
    ...
    private void addSystemImes(Set<String> excludePackages) {
        ...
        for (InputMethodInfo imi : imis) {
            try {
                Log.d("YSY", "unexclude ime app : " + imi.getPackageName());
                if (imi.isDefault(context) && isSystemPackage(imi.getPackageName())) {
                    Log.d("YSY", "exclude ime app : " + imi.getPackageName());
    ...
    private void addSystemApps(List<SelectableAppInfo> visibleApps, Intent intent,
        ...
        for (ResolveInfo app : launchableApps) {
            if (app.activityInfo != null && app.activityInfo.applicationInfo != null) {
                ...
                Log.d("YSY", "All of luncher status app list : " + app.activityInfo.name);
                if ((flags & ApplicationInfo.FLAG_SYSTEM) != 0
                        || (flags & ApplicationInfo.FLAG_UPDATED_SYSTEM_APP) != 0) {
                    // System app
                    ...
                    Log.d("YSY", "first include system app list : " + app.activityInfo.name);

                    // [START][2014-01-09][seungyeop.yeom@lge.com]
                    // except of Settings app
                    String appName = app.activityInfo.name;
                    if (appName.equals(info.packageName
                            + ".TetherSettingsShortCutLauncherSupport")
                            || appName
                                    .equals(info.packageName
                                            + ".DataUsageSummaryShortCutLauncherSupport")
                            || appName
                                    .equals(info.packageName
                                            + ".PowerSaveBatteryShortCutLauncherSupport")
                            || appName
                                    .equals(info.packageName
                                            + ".users.KidsModeMainActivity")
                            || appName.equals(info.packageName
                                    + ".simplesettings.SimpleSettings")) {
                        Log.d("YSY", "except of Settings app, appName : " + appName);
                        continue;
                    }
                    // except of call app
                    if (appName.equals(info.packageName + ".activities.DialtactsActivity")
                            || appName.equals(info.packageName
                                    + ".DialtactsRecentCallsEntryActivity")) {
                        Log.d("YSY", "except of call app, appName : " + appName);
                        continue;
                    }
                    // [END]

                    // [START][2014-01-09][seungyeop.yeom@lge.com] except without the app name
                    if (info.activityName.equals("")) {
                        Log.d("YSY", "except without the app name , appName : " + appName);
                        continue;
                    }
                    // [END]
    ...
    private void fetchAndMergeApps() {
        ...
        for (ApplicationInfo app : installedApps) {
            ...
            if ((app.flags & ApplicationInfo.FLAG_SYSTEM) == 0
                ...
                Log.d("YSY", "Downloaded app (not system app) : " + app.packageName);
                ...
                try {
                    ...
                    Log.d("YSY", "System app (not Download app) : " + app.packageName);
                    if (mRestrictedProfile
                            && pi.requiredAccountType != null && pi.restrictedAccountType == null) {
                        mSelectedPackages.put(app.packageName, false);
                        Log.d("YSY",
                                "System app (requiredAccountType != null && restrictedAccountType == null) : "
                                        + app.packageName);
                    }
        ...
        if (mUserApps != null) {
            for (ApplicationInfo app : mUserApps) {
                ...
                if ((app.flags & ApplicationInfo.FLAG_SYSTEM) == 0
                    ...
                    info.activityName = info.appName;
                    info.icon = app.loadIcon(pm);
                    Log.d("YSY", "Download app (other user) : " + app.packageName);
        ...
        // [START][seungyeop.yeom] delete apps of visible list for restricted profile
        for (int i = mVisibleApps.size() - 1; i >= 0; i--) {
            SelectableAppInfo info = mVisibleApps.get(i);
            if (info.packageName.equals("com.lge.music")
                    || info.packageName.equals("com.lge.app.richnote")
                    || info.packageName
                            .equals("com.lge.sizechangable.worldclock")
                    || info.packageName.equals("com.lge.smarttext")
                    || info.packageName.equals("com.lge.ia.task.textlink")
                    || info.packageName.equals("com.lge.wernicke.textlink")
                    || info.packageName.equals("com.android.gallery3d")
                    || info.packageName.equals("com.lg.apps.cubeapp")) {

                Log.d("YSY", "remove package : " + info.packageName);
                mVisibleApps.remove(i);
            }
        }
        // [END]
        ...
        for (SelectableAppInfo info : mVisibleApps) {
            if (packageMap.containsKey(info.packageName)) {
                ...
                Log.d("YSY", "master/slave relationship : " + info.appName);
            } else {
                packageMap.put(info.packageName, info);
                Log.d("YSY", "not master/slave relationship : " + info.appName);
            }
        }
    }

    private boolean isPlatformSigned(PackageInfo pi) {
        return (pi != null && pi.signatures != null && mSysPackageInfo.signatures[0]
    ...
    private void populateApps() {
        ...
        // [START][seungyeop.yeom][2014-05-13] delete apps of Home category in the Kids Mode
        Intent mHomeCategoryIntent = new Intent();
        mHomeCategoryIntent.setAction("android.intent.action.MAIN");
        mHomeCategoryIntent.addCategory("android.intent.category.HOME");

        List<ResolveInfo> rList = pm.queryIntentActivities(mHomeCategoryIntent,
        ...
        if (mVisibleApps.size() > 0) {
            for (SelectableAppInfo app : mVisibleApps) {
                ...
                Log.d("YSY", "populateApps(), mVisible : " + app.packageName);
                if (packageName == null) {
                    continue;
                }
                ...
                if (app.masterEntry != null) {
                    ...
                    Log.d("YSY", "populateApps(), app.masterEntry != null : " + app.packageName);
                }
                ...
                if (pi != null && (pi.requiredForAllUsers || isPlatformSigned(pi))) {
                    if (!packageName.equals("com.lge.sizechangable.weather.theme.optimus")) {
                        // [seungyeop.yeom] condition of weather.theme.optimus (no weather theme)
                        p.setChecked(true);
                        p.setImmutable(true);
                    } else {
                        // [seungyeop.yeom] condition of weather.theme.optimus (weather theme)
                        if (!mNewUser && isAppEnabledForUser(pi)) {
                            Log.d("YSY", "!mNewUser && isAppEnabledForUser(pi) : " + packageName);
                            p.setChecked(true);
                        }
                    }

                    Log.d("YSY", "populateApps(), requiredForAllUsers || isPlatformSigned()) : "
                            + app.packageName);
                    // If the app is required and has no restrictions, skip showing it
                    if (!hasSettings && !isSettingsApp) {
                        Log.d("YSY", "populateApps(), !hasSettings && !isSettingsApp) : "
                                + app.packageName);
                        // [START][seungyeop.yeom] app list of default off for restricted profile
                        if (packageName.equals("com.lge.bnr")
                                || packageName.equals("com.lge.updatecenter")
                                || packageName
                                        .equals("com.lge.sizechangable.weather.theme.optimus")
                                || packageName
                                        .equals("com.android.mms")) {
                            Log.d("YSY", "!packageName(bnr, update, weather theme : "
                                    + packageName);
                        } else {
                            continue;
                        }
                    ...
                    Log.d("YSY", "!mNewUser && isAppEnabledForUser(pi) : "
                            + packageName);
                    p.setChecked(true);
                }
                if (mRestrictedProfile
                        && pi.requiredAccountType != null && pi.restrictedAccountType == null) {
                    Log.d("YSY",
                            "populateApps(), requiredAccountType != null && pi.restrictedAccountType == null) : "
                                    + app.packageName);
                    p.setChecked(false);
                    p.setImmutable(true);
                ...
                if (mRestrictedProfile && pi.restrictedAccountType != null) {
                    Log.d("YSY", "populateApps(), restrictedAccountType != null : "
                            + app.packageName);
                ...
                // [START][seungyeop.yeom][2014-05-13] delete item of camera app for Kids mode in the list
                ...
                if ((userinfo.flags & UserInfoConstants.FLAG_KIDS) == UserInfoConstants.FLAG_KIDS) {
                    if (packageName.equals("com.lge.camera")
                            || packageName.equals("com.lge.voicerecorder")
                            || packageName.equals("com.android.calculator2")
                            || packageName.equals("com.android.calendar")) {
                        Log.d("YSY",
                                "Kids mode camera app, voice recorder app, cal app, calendar app");
                        p.setChecked(true);
                        p.setImmutable(true);
                        continue;
                    }
                    ...
                    for (ResolveInfo info : rList) {
                        if (info.activityInfo.packageName.equals(packageName)) {
                            Log.d("yeom3", "info.activityInfo.packageName.equals(packageName) : "
                ...
                // [START][seungyeop.yeom] app list of default off for restricted profile
                if (packageName.equals("com.lge.bnr") || packageName.equals("com.lge.p2p")
                        || packageName.equals("com.lge.updatecenter")
                        || packageName.equals("com.lge.pa")
                        || packageName.equals("com.android.mms")) {
                    Log.d("YSY", "default off for restricted profile : "
                            + app.packageName);
                    p.setChecked(false);
                    p.setImmutable(true);
                    p.setSummary(R.string.app_not_supported_in_limited);
                ...
                Log.d("YB", "Total visible list : " + app.packageName);
