...
public class BatteryHistoryChart extends View {
    ...
    final BatteryChartData mPhoneSignalChart = new BatteryChartData();
    ...
    /*LGE_CHANGE_S */
    static final int HIGHT_MARGIN = 5;
    static final int LARGE_MODE_TEXT_SIZE_MARGIN = 4;
    int mTextSize;
    ...
    public BatteryHistoryChart(Context context, AttributeSet attrs) {
        super(context, attrs);
        mTextSize = 0;
        // jongtak0920.kim 120817 Edit Graph [START]
        // mBatteryBackgroundPaint.setARGB(255, 128, 128, 128);
        mBatteryBackgroundPaint.setARGB(128, 121, 232, 255);
        // jongtak0920.kim 120817 Edit Graph [END]
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.BatteryHistoryChart_android_shadowColor) {
                shadowcolor = a.getInt(attr, 0);
            } else if (attr == R.styleable.BatteryHistoryChart_android_shadowDx) {
                dx = a.getFloat(attr, 0);
            } else if (attr == R.styleable.BatteryHistoryChart_android_shadowDy) {
                dy = a.getFloat(attr, 0);
            } else if (attr == R.styleable.BatteryHistoryChart_android_shadowRadius) {
                r = a.getFloat(attr, 0);
            } else if (attr == R.styleable.BatteryHistoryChart_android_textColor) {
                textColor = a.getColorStateList(attr);
            } else if (attr == R.styleable.BatteryHistoryChart_android_textSize) {
                textSize = a.getDimensionPixelSize(attr, textSize);
            } else if (attr == R.styleable.BatteryHistoryChart_android_typeface) {
                typefaceIndex = a.getInt(attr, typefaceIndex);
            } else if (attr == R.styleable.BatteryHistoryChart_android_textStyle) {
        ...
        if (textColor != null) { //yonguk.kim 20120509 WBT Issue
            mTextPaint.setColor(textColor.getDefaultColor());
        }

        /*LGE_CHANGE_S */
        mTextSize = textSize - 3;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (mLargeMode) {
            //LGE_CHANGE_S
            int barOffset = textHeight + mLineWidth + HIGHT_MARGIN;
            //LGE_CHANGE_E
        ...
        if (mStats.startIteratingHistoryLocked()) {
            ...
            while (mStats.getNextHistoryLocked(rec) && i < N) {
                if (rec.cmd == BatteryStats.HistoryItem.CMD_UPDATE) {
                    ...
                    y = mLevelTop + levelh - ((rec.batteryLevel - batLow) * (levelh - 1))
                            / batChange;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mLargeMode) {
            ...
            if (mDurationStringWidth > width - 15) {
                String durationString = Utils.formatElapsedTime(getContext(),
                        mStatsPeriod / 1000, true);
                mDurationString = durationString; //getContext().getString(R.string.battery_stats_on_battery, durationString);
            }
            ...
            /*LGE_CHANGE_S */
            mTextPaint.setTextSize(mTextSize + LARGE_MODE_TEXT_SIZE_MARGIN);
            mDurationStringWidth = (int)mTextPaint.measureText(mDurationString);
            mTotalDurationStringWidth = (int)mTextPaint.measureText(mTotalDurationString);
            mTextAscent = (int)mTextPaint.ascent();
            mTextDescent = (int)mTextPaint.descent();
            ...
            // yonguk.kim 20120525 Extra large case issue fix
            ...
            float withText = mTextPaint.measureText(mDurationString);
            canvas.drawText(mDurationString, (width / 2) - (withText / 2),
                    (height / 2) - ((mTextDescent - mTextAscent) / 2) - mTextAscent, mTextPaint);

            //            if((width/2) - (mDurationStringWidth/2) > 0 ){
            //                canvas.drawText(mDurationString, (width/2) - (mDurationStringWidth/2),
            //                        (height/2) - ((mTextDescent-mTextAscent)/2) - mTextAscent, mTextPaint);
            //            }else{
            //                canvas.drawText(mDurationString, 10,
            //                        (height/2) - ((mTextDescent-mTextAscent)/2) - mTextAscent, mTextPaint);
            //            }
        }
        ...
        if (mLargeMode) {
            if (mHavePhoneSignal) {
                canvas.drawText(mPhoneSignalLabel, 0,
                        height - mPhoneSignalOffset - mTextDescent - HIGHT_MARGIN, mTextPaint);
            }
            if (mHaveGps) {
                canvas.drawText(mGpsOnLabel, 0,
                        height - mGpsOnOffset - mTextDescent - HIGHT_MARGIN, mTextPaint);
            }
            if (mHaveWifi) {
                canvas.drawText(mWifiRunningLabel, 0,
                        height - mWifiRunningOffset - mTextDescent - HIGHT_MARGIN, mTextPaint);
            }
            canvas.drawText(mWakeLockLabel, 0,
                    height - mWakeLockOffset - mTextDescent - HIGHT_MARGIN, mTextPaint);
            canvas.drawText(mScreenOnLabel, 0,
                    height - mScreenOnOffset - mTextDescent - HIGHT_MARGIN, mTextPaint);
