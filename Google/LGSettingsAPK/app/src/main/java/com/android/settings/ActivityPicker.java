...
//import android.content.thm.ThemeIconManager;
...
public class ActivityPicker extends AlertActivity implements
    ...
    // CAPP_THEMEICON
    protected boolean mWantNoThemeIcon = false;
    private static Method sUseThemeIconMethod = null;
    private static Method sSetThmMethod = null;
    private static Method sGetThmMethod = null;
    static {
        try {
            Class clsThemeIconManager = Class.forName("android.content.thm.ThemeIconManager");
            sUseThemeIconMethod = clsThemeIconManager.getMethod("isUseThemeIcon",
                    new Class[] { IPackageManager.class, String.class, boolean.class });
            sSetThmMethod = Resources.class.getMethod("setThemeIconManager", clsThemeIconManager);
            sGetThmMethod = Resources.class.getMethod("getThemeIconManager", (Class<?>[])null);
        } catch (Exception e) {
        }
    }
    // CAPP_THEMEICON_END
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // CAPP_THEMEICON
        mWantNoThemeIcon = false;
        if (getCallingPackage() != null) {
            if (sUseThemeIconMethod != null && sSetThmMethod != null && sGetThmMethod != null) {
                try {
                    boolean useThemeIcon = (Boolean)sUseThemeIconMethod.invoke(null,
                            new Object[] {
                                IPackageManager.Stub.asInterface(ServiceManager.getService("package")),
                                getCallingPackage(), false });

                    mWantNoThemeIcon = !useThemeIcon;
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
        ...
        if (labels != null && icons != null && labels.size() == icons.size()) {
            for (int i = 0; i < labels.size(); i++) {
                ...
                try {
                    ...
                    // CAPP_THEMEICON
                    if (mWantNoThemeIcon) {
                        icon = loadNoThemeIcon(packageManager,
                                iconResource.packageName, iconResource.resourceName, 0);
                    } else {
    ...
    // CAPP_THEMEICON
    protected static Drawable loadNoThemeIcon(PackageManager pm, String packageName,
            String resourceName, int icon) {
        Drawable dr = null;
        try {
            Resources res = pm.getResourcesForApplication(packageName);
            // detach
            Object thm = sGetThmMethod.invoke(res, (Object[])null);
            sSetThmMethod.invoke(res, (Object)null);
            try {
                if (resourceName != null) {
                    icon = res.getIdentifier(resourceName, null, null);
                }
                if (icon != 0) {
                    dr = res.getDrawable(icon);
                }
            } catch (Resources.NotFoundException e) {
                // Ignore
            }
            // MUST attach again!
            sSetThmMethod.invoke(res, thm);
        } catch (NameNotFoundException e) {
            // Ignore
        } catch (Exception e) {
            // Ignore
    ...
    // refer to loadIcon in ResolveInfo
    protected static Drawable loadNoThemeIcon(PackageManager pm, ResolveInfo ri) {
        Drawable dr;
        if (ri.resolvePackageName != null && ri.icon != 0) {
            dr = loadNoThemeIcon(pm, ri.resolvePackageName, null, ri.icon);
            if (dr != null) {
                return dr;
            }
        }
        ...
        if (ci != null) {
            ...
            if (ri.icon != 0) {
                dr = loadNoThemeIcon(pm, ci.packageName, null, ri.icon);
                if (dr != null) {
                    return dr;
                }
            }
            int icon = ci.getIconResource();
            if (icon != 0) {
                dr = loadNoThemeIcon(pm, ci.packageName, null, icon);
                if (dr != null) {
                    return dr;
                }
            }
    ...
    protected void putIntentItems(Intent baseIntent, List<PickAdapter.Item> items) {
        ...
        for (int i = 0; i < listSize; i++) {
            ...
            items.add(new PickAdapter.Item(this, packageManager, resolveInfo, mWantNoThemeIcon)); // CAPP_THEMEICON
        }
    }
    ...
    protected static class PickAdapter extends BaseAdapter {
        ...
        public static class Item implements AppWidgetLoader.LabelledItem {
            ...
            Item(Context context, PackageManager pm, ResolveInfo resolveInfo) {
                if (resolveInfo != null) {
                    ...
                    if (null != resolveInfo.activityInfo) {
                        if (null != resolveInfo.activityInfo.applicationInfo) {
            ...
            // CAPP_THEMEICON
            Item(Context context, PackageManager pm, ResolveInfo resolveInfo, boolean wantNoThemeIcon) {
                label = resolveInfo.loadLabel(pm);
                if (label == null && resolveInfo.activityInfo != null) {
                    label = resolveInfo.activityInfo.name;
                }

                icon = getResizer(context).createIconThumbnail(
                        (wantNoThemeIcon) ? loadNoThemeIcon(pm, resolveInfo) :
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            textView.setCompoundDrawablesRelativeWithIntrinsicBounds(item.icon, null, null, null);
